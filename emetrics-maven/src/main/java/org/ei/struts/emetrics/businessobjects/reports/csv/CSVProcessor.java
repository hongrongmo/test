...
public class CSVProcessor {
    ...
    public List executeQuery(ResultsSet resultsSet, Map pStmntValues, ResultProcessor processor) throws Exception {

        List results = new ArrayList();
        String filename = ((String) pStmntValues.get("path")).concat((String) pStmntValues.get("filename"));
        try {
    ...
    protected List handleQuery(ResultsSet resultsSet, Map pStmntValues, ResultProcessor processor) throws IOException {
        ...
        try {
            ...
            if (metadata == null || metadata.getContentLength() == 0) {
                throw new IOException("No report available for: " + key);
            } else {
                log.info("File " + key + " successfully opened. Last modified = " + metadata.getLastModified());
            }
            in = new BufferedReader(new InputStreamReader(s3service.getReportFromS3Bucket(key)));
            // in = new BufferedReader(new FileReader(inputFile));


            Report report = resultsSet.getReport();
            report.setTimestamp(metadata.getLastModified().getTime());
            ...
            if ((report.getReportId() == Constants.MARKETER_DATABASE_COMBO) || (report.getReportId() == Constants.CUSTOMER_DATABASE_COMBO)) {
                report.setTableRows(new ArrayList());
                resultsSet = report.createResultsSet(report.getReportId() + 1);
                int resultIdbase = resultsSet.getResultsSetId() * 10;
                Result result = resultsSet.createResult(resultIdbase);
            ...
            if ((report.getReportId() == Constants.COUNTER2_DATABASE_REPORT1_ID)) {
                ...
                while (iter.hasNext()) {
                    ...
                    try {
                        ...
                        long colval = Long.parseLong(sumBean.get(colname).toString());
                        ...
                        if (colval == 0) {
                            ...
                            while (itrRows.hasNext()) {
                                Map arowmap = ((LazyDynaBean) itrRows.next()).getMap();
                                arowmap.remove(colname);
                            }
                        }
                    } catch (NumberFormatException e) {
                        log.error("error parsing " + sumBean.get(colname).toString(), e);
                    }
                } // while
            } // if

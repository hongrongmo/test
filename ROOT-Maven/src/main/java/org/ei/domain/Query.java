...
public class Query implements Comparable<Object>, Serializable {
	...
	public void setSearchPhrase(HttpServletRequest request) {
		setSeaPhr1(getRequestParameter(request, "searchWord1"));
		setSeaOpt1(getRequestParameter(request, "section1"));
		setBool1(getRequestParameter(request, "boolean1"));
		setSeaPhr2(getRequestParameter(request, "searchWord2"));
		setSeaOpt2(getRequestParameter(request, "section2"));
		setBool2(getRequestParameter(request, "boolean2"));
		setSeaPhr3(getRequestParameter(request, "searchWord3"));
		setSeaOpt3(getRequestParameter(request, "section3"));
		setSearchWords(request.getParameterValues("searchWords"));
		setBooleans(request.getParameterValues("booleans"));
		setSections(request.getParameterValues("section"));
	...
	private String getRequestParameter(HttpServletRequest request, String paraName) {
	...
	/**
	 * Create Events that need to be recorded based on what might have changed
	 * on the search form. Things like the limit to section and sort by.
	 * 
	 * @return
	 */
	public ArrayList<GoogleWebAnalyticsEvent> createQueryEventList() {
		ArrayList<GoogleWebAnalyticsEvent> eventList = new ArrayList<GoogleWebAnalyticsEvent>();
		GoogleWebAnalyticsEvent theEvent = null;

		if ((sDocType != null) && (sDocType.length() > 0) && !(sDocType.equals("NO-LIMIT"))) {
		...
		if (sTreatmentType.length() > 0 && !(sTreatmentType.equals("NO-LIMIT"))) {
			theEvent = new GoogleWebAnalyticsEvent();
			theEvent.setCategory(WebAnalyticsEventProperties.CAT_LIMIT_TO);
		...
		if ((sDisciplineType != null) && (sDisciplineType.length() > 0) && !(sDisciplineType.equals("NO-LIMIT"))) {
			theEvent = new GoogleWebAnalyticsEvent();
			theEvent.setCategory(WebAnalyticsEventProperties.CAT_LIMIT_TO);
		...
		if (sLanguage != null && sLanguage.length() > 0 && (!sLanguage.equals("NO-LIMIT"))) {
			theEvent = new GoogleWebAnalyticsEvent();
			theEvent.setCategory(WebAnalyticsEventProperties.CAT_LIMIT_TO);
		...
		if (Sort.PUB_YEAR_FIELD.equals(getSortOption().getSortField())) {
			theEvent = new GoogleWebAnalyticsEvent();
		...
		if (sAutoStemming.equalsIgnoreCase(OFF)) {
			theEvent = new GoogleWebAnalyticsEvent();
			theEvent.setCategory(WebAnalyticsEventProperties.CAT_SORT_BY);

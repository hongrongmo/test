...
public class MoreSearchSources
{
    ...
    private int[] dbMasks = {org.ei.domain.DatabaseConfig.CBF_MASK,
	 ...
	 * @param 
	 * @return
	 *  Links-- Referex, CRC ENGnetBASE, ReedLink, GlobalSpec, IHS Standards, USPTO, esp@cenet, Scirus, EEVL, LexisNexis News
	    //The order here will affect the order in the "More Search Sources" box on the site! Local Databases come first
        // The order of these "More Search Sources" is determined here as they are added to the output buffer.
        // BOOK SEARCH-- "PAG","CRC","EMS","GSP"
	    // PATENTS-- "UPO","ESN","SCI","EEV"
	    // LEXISNEXIS NEWS-- "LEX"
        //  Right now it is always hardcoded for all userslater on, if it is added in the user cartridge, it can be retrieved by following code
		//  LEX USer Cartridge LEX is a 'negative' cartridge.  So if the does NOT have the cartridge, include LexisNexis News in the output.
	    ...
	    if(lstCarts.size() > 0)
	    {
	        ...
	        { 
	            if((userMask & dbMasks[x]) == dbMasks[x])
	            {
	                ...
	                if(dbMasksNameValues[0]!=null && !dbMasksNameValues[0].equals("null") 
	                		&& dbMasksNameValues[1]!=null  && !dbMasksNameValues[1].equals("null")){
	        ...
	        {  
	        	...
	        	}else if(!userprefs.getMoreSearchSources(links[listsize]) && ((lstCarts.contains(links[listsize]))) && dataBaseNames.get(links[listsize]) != null){
	            ...
	            if(dbNameValues!=null){
	            	 ...
	            	 if(dbNameValues[0]!=null && !dbNameValues[0].equals("null") 
		                		&& dbNameValues[1]!=null  && !dbNameValues[1].equals("null")){
	...
	public void addMoresources(String[] dbmoreresources) {
    	if (this.moresources == null) this.moresources = new HashMap();

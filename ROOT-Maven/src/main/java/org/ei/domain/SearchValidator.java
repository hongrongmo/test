...
public class SearchValidator {
	 ...
	 * @return
	 */
	public SearchValidatorStatus validate(String searchtype, int databasemask, UserSession ussession, ISearchForm searchform) {
		log4j.info("Validating search form for searchtype: " + searchtype);

		DatabaseConfig databaseConfig = DatabaseConfig.getInstance();
		String[] credentials = ussession.getUser().getCartridge();
		...
		try {
			...
			queryObject.setSessionID(ussession.getID());
			queryObject.setUserID(ussession.getUser().getUserId());
	 ...
	 * @return
	 */
	public SearchValidatorStatus validate(String rerunid, String searchtype, int databasemask, UserSession ussession, ISearchForm searchform) {
		try {
			//
			...
			queryObject.setCredentials(ussession.getUser().getCartridge());

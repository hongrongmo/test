...
/**
 * This class used to get Customized Features for the user like the
 * databases,year range,classification codes, local holdings and email alerts.
 */
...
public class ClientCustomizer {
	...
	/**
	 * Constructor which takes the customer id of the user and gets all the
	...
	/**
	 * This method creates a string that will contain the start year for each
	 * database which the user subscribes to and also a selected start year for
	 * each database. The start yeaar will be the furthest back a search can go
	 * and the selected start year will appear as the default in all search
	 * forms.
	 * 
	 * i.e. CSY1960CST1884ISY1970IST1969NSY1980NST1899
	 * 
	 * @return String which contains keys followed by year values for each
	 *         database the user subscribes to
	 * @throws ClientCustomizerException
	 */
	private String getClientStartYears(String carstr, String[] cartridges)
			throws ClientCustomizerException {
		if (GenericValidator.isBlankOrNull(carstr) || cartridges == null
				|| cartridges.length == 0) {
			throw new ClientCustomizerException(
					"Empty cartridge string passed to getClientStartYears!");
		}
		...
		for (int y = 0; y < d.length; y++) {
			if (d[y] != null) {
				...
				int dbStartYear = dbconfig.getStartYear(cartridges,
						d[y].getMask());

				buf.append(skey);
				if (carstr.indexOf(skey) > -1) {
					buf.append(carstr.substring(carstr.indexOf(skey) + 3,
							carstr.indexOf(skey) + 7));
				} else {
	...
	/**
	 * This method checks whether the user has rights to access local holdings
	 * or not.
	 * 
	 * @return true - if the user can access/false otherwise.
	 * @exception ClientCustomizerException.

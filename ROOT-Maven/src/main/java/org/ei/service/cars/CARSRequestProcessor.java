...
public class CARSRequestProcessor {
	...
	private CARSResponse buildResponse(CARSRequest carsRequest, HttpServletRequest httprequest) throws ServiceException {
		...
		//
		Resource resource = RESTResourceBuilder.build(carsRequest, httprequest);
		if (resource == null) {
		...
		try {
			...
			case PUT:
				...
				if (carsRequest.getRequestURI().contains(
					...
					PutMethod putMethod = RESTResourceBuilder.buildPutMethod(carsRequest, httprequest);
					putMethod.setRequestHeader(CARSStringConstants.ACCEPT_ENCODING.value(), CARSConstants.GZIP);
					putMethod.setRequestEntity(new MultipartRequestEntity(RESTResourceBuilder.fetchParts(carsRequest, httprequest), putMethod.getParams()));

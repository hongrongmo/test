...
import org.ei.referex.ReferexURLMap;
...
public class OpenURL extends HttpServlet {
  ...
  public void service(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    /** get all parameters through OpenURL currently we support 9 out of 25 fields, will add more later
     */

    /** genre(DT), aulast(AU), issn(SN), coden(CN), isbn(BN), title(ST), atitle(TI), stitle(ST), date(YR) terms (ALL) - not supported by OpenURL Spec
     */

    /** 10/21/03 added issue(SU), volume(VO), spage(SP) total fields supported: 12
     */
    ...
    try
    {
        ...
        if(genre != null &&  genre.equals("book"))
        {
            //System.out.println("*********Redirecting To Quick Search Book Results************");
        	//since all books are moving to ScienceDirec we will be redirecting this request to a temporary redirect action and page telling the user what 
        	//we are doing
        	ReferexURLMap referexMap = ReferexURLMap.getInstance();
        	String referexIsbn = prep(request.getParameter("isbn"));
        	String redirect = referexMap.getRedirectUrl(referexIsbn);
        	if(StringUtils.isNotBlank(redirect)){
        		response.sendRedirect("http://"+serverName + "/bookredirect.url?url=" + redirect);
        	}else{
        		if(StringUtils.isBlank(referexIsbn)){referexIsbn = "";}
        		response.sendRedirect("http://"+serverName +SystemMessage.SYSTEM_ERROR_URL+ "?message=Error Code: " +  SystemErrorCodes.REFEREX_ISBN_NOT_MAPPED + " - Redirect URL for isbn " + referexIsbn + " does not exist!");
        		log("Error Getting Redirect URL for isbn " + referexIsbn);
        	}
        } else {
            if(genre != null){
          ...
          response.sendRedirect("http://"+serverName+"/search/results/expert.url?CID=expertSearchCitationFormat&SYSTEM_PT=t&searchtype="+Query.TYPE_EXPERT+"&database="+dbMask+"&searchWord1="+URLEncoder.encode(searchWord.toString())+"&yearselect=yearrange&startYear="+startYear+"&endYear="+endYear);
		}
  ...
  private String prep(String s)
  {
      ...
      //removing for redirect to scidir doesn't need the * 
//      if(s.indexOf("*") == -1)
//      {
//          s = s+"*";
//      }

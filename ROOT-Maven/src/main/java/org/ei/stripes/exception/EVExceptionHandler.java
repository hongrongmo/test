...
/**
 * This class handles all exceptions for the EV web application.
 * NOTE that it forwards to /WEB-INF/pages/world/exception.jsp -
 * it should NOT forward to any layout using standard.jsp because
 * it relies on the EVActionBean hierarchy.
 *
 * @author harovetm
 *
 */
public class EVExceptionHandler extends DefaultExceptionHandler{
	...
	public Resolution handleBasicException(EVBaseException exc, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (actionbean != null) {
            ...
            actionbean.createWebEvent(GA_EXCEPTION_CATEGORY, exc.getClass().getSimpleName(), "Error Code: " + Integer.toString(exc.getErrorCode()) + ", Instance ID: " + new AWSInfo().getEc2Id() + ", Message: " + exc.getMessage());
        }
	...
	/*ExceptionHandler implementation for system error message but to have this
	 *working we have to change all the redirections and also CID check would not
	 *work. Since CID would be always the action Bean which is throwing the exception
	 *to handle that we need to throw the specific the exceptions and define handler for
	 *each exception. Each handler will have its own parameter set for JSP.
	 *
	...
	@Override
	public void handle(Throwable exc, HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException {


		String cid = request.getParameter("CID");

		if(cid != null && cid.equalsIgnoreCase("invalidcid")){
			request.setAttribute("pageTitle", "System Error");
			invalidcid = true;
		}

		if(cid != null && cid.equalsIgnoreCase("endsession")){
			request.setAttribute("pageTitle", "Session Ended");
			request.getSession(true);
			endsession = true;
		}

		if(cid != null && cid.equalsIgnoreCase("error")){
			request.setAttribute("pageTitle", "Engineering Village - System Error");
			error = true;

...
public abstract class EVActionBean implements ActionBean, ISecuredAction {
	...
	/**
	 * Override for the ISecuredAction interface.  By default all ActionBeans require
	 * either Individual or Guest auth access.  Action Beans with different requirements
	 * (e.g. no access control or just Individual) should override this method!
	 */
	@Override
	...
	/**
	 * Create a timestamp for request start.  This is mainly
	 * used for the Google Analaytics call in standard.jsp.
	 */
	 ...
	 * WARNING!!!!!
	 *
	 * At the completion of every request re-write the session cookie.  We do
	 * this in order to handle changes to the user session. If the session
	 * has been updated then the version number will have changed.  This
	 * signals other JVMs that their SessionCache object is out-of-date and
	 * needs to be updated.
	 */
	...
	public void setSessionCookie() {
        ...
        if (request != null && !GenericValidator.isBlankOrNull(request.getParameter(SessionManager.REQUEST_PT))) {
            try {
                EVSessionListener.cleanSessionTables(context.getUserSession().getSessionID().getID());
            } catch (Exception e) {
	...
	public boolean isRoomSearch() { return this.room == ROOM.search;}
	public boolean isRoomSelectedRecords() { return this.room == ROOM.selectedrecords;}
	public boolean isRoomMySettings() { return this.room == ROOM.mysettings;}
	public boolean isRoomTagsGroups() { return this.room == ROOM.tagsgroups;}
	public boolean isRoomBulletins() { return this.room == ROOM.bulletins;}
	 ...
	 * @param resourcekey
	 */
	protected void addGlobalError(HttpServletRequest request,
			String resourcekey, Object... resourceparms) {
    ...
    private boolean isValidNextUrl(String nextURL) {
        ...
        if (StringUtils.isBlank(nextURL)) return false;

        try {
            // False if URL is one of the start page URLs
            String decode = URLDecoder.decode(nextURL, "UTF-8");
            if (decode.contains(getStartPage()) ||
                decode.contains(EVPathUrl.EV_HOME.value())) {
                return false;
            }
    ...
    /**
	 * The next 2 methods work together to produce Usage logging.
	 * The startlog() method will set the current time for the
	 * start and finalizelog() will set then ending time and
	 * enqueue to the Logger.
	 *
	 */
	...
	protected String getContentDispositionFilenameTimestamp(String baseFilename) throws UnsupportedEncodingException {
		java.util.Calendar calCurrentDate = java.util.GregorianCalendar
				.getInstance();
		StringBuffer strbFilename = new StringBuffer();
		strbFilename
				.append(calCurrentDate.get(java.util.Calendar.DAY_OF_MONTH));
		strbFilename.append("-");
    ...
    protected void addWebEvent(GoogleWebAnalyticsEvent	webEvent){
    	ArrayList<GoogleWebAnalyticsEvent> eventList = (ArrayList<GoogleWebAnalyticsEvent>) context.getRequest().getAttribute(WebAnalyticsEventProperties.WEB_EVENT_REQUEST_NAME);
     ...
     * @param webEvents
     */
    protected void appendWebEventList(ArrayList<GoogleWebAnalyticsEvent>	webEvents){
    	ArrayList<GoogleWebAnalyticsEvent> eventList = (ArrayList<GoogleWebAnalyticsEvent>) context.getRequest().getAttribute(WebAnalyticsEventProperties.WEB_EVENT_REQUEST_NAME);

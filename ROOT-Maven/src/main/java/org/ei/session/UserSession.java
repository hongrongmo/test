 ...
 * because this object is eligible to be written to a data store via serialization.  In order
 * to keep the size of it to a minimum, the items related to authentication (entitlements,
 * text zones, etc.) are NOT serialized.
 * 
 * Tomcat offers a PersistenceManager that can be configured in the application's context.xml
 * to write session information to a data store.  For EV this can be enabled by adding the
 * following to WebContent/META-INF/context.xml:
 * 
 *  &lt;Manager className="org.apache.catalina.session.PersistentManager" saveOnRestart="true" maxIdleBackup="20" minIdleSwap="-1" maxIdleSwap="-1" maxInactiveInterval="1800"&gt;
 *  &lt;Store className="org.apache.catalina.session.JDBCStore"
 *      dataSourceName="jdbc/session" sessionAppCol="app_name"
...
public class UserSession implements Serializable 
{
	...
	/**
	 * Loads session information from a properties object.  This is only used
	 * in the /engvillage project to restore the session values from incoming
	 * request.
	 * 
	 * @param props
	 */
	...
	/**
	 * This is the counterpart to loadFromProperties.  This is used before transferring
	 * session information to the /engvillage application
	 * @return
	 */
	public Properties unloadToProperties() {
		Properties props = new Properties();
		if (this.sessionID != null) props.put(SESSIONID_KEY, this.sessionID.toString());
		if (this.user != null) props.put(USER_KEY, this.user.toString());
		props.put(LASTTOUCHED_KEY, Long.toString(this.lastTouched));
		props.put(EXPIREIN_KEY, Long.toString(this.expireIn));
		if (this.status != null) props.put(STATUS_KEY, this.status);
    ...
    public Set<UserEntitlement> filterUserEntitlements(ENTITLEMENT_TYPE entitlementtype) {
        if (this.userEntitlements == null) return Collections.EMPTY_SET;
    ...
    public void addUserEntitlements(Set<UserEntitlement> userEntitlements) {
        if (this.userEntitlements == null) userEntitlements = new HashSet<UserEntitlement>();
    ...
    public void removeUserEntitlements(List<String> userEntitlements) {
        if (this.userEntitlements != null) this.userEntitlements.removeAll(userEntitlements);
    }
    
    public void addUserTextZones(Map<String,String> textzones) {
        if (this.userTextZones == null) userTextZones = new HashMap<String,String>();
    ...
    public Map<String, String> getUserTextZones() {
        if (userTextZones == null) userTextZones = new HashMap<String,String>();
        return userTextZones;
    }
    
    public Set<UserEntitlement> getUserEntitlements() {
        if (userEntitlements == null) userEntitlements = new HashSet<UserEntitlement>();
	...
	public void setEnvSessionEndRedir(String envSessionEndRedir) {
        if (envSessionEndRedir == null) return;
	...
	public void setLocalHoldingKey(String localHoldingKey) {
        if (localHoldingKey == null) return;
	...
	public void setTokenConcurrent(String TokenConcurrent) {
        if (TokenConcurrent == null) return;

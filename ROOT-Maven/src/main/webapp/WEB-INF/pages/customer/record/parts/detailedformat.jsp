...
<c:when test="${'AN' eq segment.key and not empty result.accnum}"> <!--Accession Number  -->
			<tr>
					<td style="white-space: nowrap"><span style="font-weight: normal;font-size: 14px">${result.doc.hitindex}. <label class="hidden" for="cbresult_${result.doc.docid}">Add to selected records</label><input type="checkbox" name="cbresult" id="cbresult_${result.doc.docid}" handle="${result.doc.hitindex}" docid="${result.doc.docid}" dbid="${result.doc.dbid}"<c:if test="${result.selected}"> checked="checked"</c:if>/></span></td>
					...
					<td colspan="2">
                        ...
                        <p><img src="/static/images/btn_aip.gif" border="0"  title="Records still in the process of being indexed, but available online" alt="Records still in the process of being indexed, but available online"><span style="vertical-align:text-top"> In Process</span>&nbsp;<a href="${actionBean.helpUrl}#Art_in_Press.htm" alt="Information about Article in Press" title="Information about Article in Press" class="helpurl"><img src="/static/images/i.png" border="0" alt=""/></a></p></c:if>
					</td>
				</tr>
</c:when>					
...
<c:when test="${'PM' eq segment.key and not empty result.pm}">
				<tr>
					<c:choose>	
						<c:when test="${empty result.accnum}">
							<td style="white-space: nowrap"><span style="font-weight: normal;font-size: 14px">${result.doc.hitindex}.  <label class="hidden" for="cbresult_${result.doc.docid}">Add to selected records</label><input type="checkbox" name="cbresult" handle="${result.doc.hitindex}" docid="${result.doc.docid}" dbid="${result.doc.dbid}"<c:if test="${result.selected}"> checked="checked"</c:if>/></span></td>
						</c:when>
						<c:otherwise>
						    <td>&nbsp;</td>
						</c:otherwise>	
...
<c:when test="${'EASM' eq segment.key and not empty result.patassigneelinks}"> 
					<tr>
					<td>&nbsp;</td>
					<td class="label"><h3>Patent assignee: </h3></td>
...
<c:when test="${'BN' eq segment.key and not empty result.isbnlink}">
				<tr>
					<td>&nbsp;</td>
...
<c:when test="${'BN13' eq segment.key and not empty result.isbn13link}">
				<tr>
					<td>&nbsp;</td>
					<td class="label"><h3>ISBN-13: </h3></td>
					<td colspan="2">${result.isbn13link}</td>
				</tr>
</c:when>
...
<c:when test="${'CF' eq segment.key and not empty result.cf}">
				<tr>
					...
					<td colspan="2"><data:highlighttag value="${result.cf}" on="${actionBean.ckhighlighting}"/></td>
				</tr>
</c:when>
<c:when test="${'MD' eq segment.key and not empty result.md}">
				<tr>
					...
					<td colspan="2"><data:highlighttag value="${result.md}" on="${actionBean.ckhighlighting}"/></td>
				</tr>
</c:when>
...
<c:when test="${'OCLS' eq segment.key and not empty result.abstractrecord.inspecclassificationcodes}">
			<%-- ************************************************************************* --%>

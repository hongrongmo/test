							...
							<c:forEach items="${result.lhlinkObjects}" var="lhlink" varStatus="status">
								<c:if test="${not empty lhlink.url}">
									...
									<c:choose>
		                        		...
		                        		<c:otherwise>
		                        			<a CLASS="LgBlueLink" onclick="openLocalHoldingsLink(event,'Citation Format',this);" href="/search/results/localholdinglinks.url?docId=${result.doc.docid}&url=${lhlink.url}&position=${lhlink.position}" target="new">${lhlink.label}</a>
		                        		</c:otherwise>
		                    		</c:choose>
								</c:if>
							</c:forEach>

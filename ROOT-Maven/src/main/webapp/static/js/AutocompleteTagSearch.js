		...
		{
			var sc = "1";
			if (document.searchtagsform.scope != null)
			{
				var len = document.searchtagsform.scope.length;
				var i = 0;
				for ( i = 0; i < len ; i++)
				{
					if (document.searchtagsform.scope[i].selected)
					{
						sc = document.searchtagsform.scope[i].value;
        ...
        function initVars() 
        {
            ...
            scope_group = findScope();      
          //  userid =  document.searchtagsform.puserid;   
            inputField = document.getElementById("searchgrouptags");
       ...
       {      
            ...
            if (inputField.value.length >0 )
            {
				...
				var url = "/controller/servlet/Controller?CID=tagAutocomplete&tag=" + escape(inputField.value)+"&scope="+escape(scope_group)+ "&timestamp="+date.getTime();

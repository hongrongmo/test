...
$(document).ready(function() {
	...
	//
	$("#addsearchllink").click(function(event) {
		event.preventDefault();
		addSearchField('');
		if(acSessionEnabled){
			checkACField($('.section:visible')[$('.section:visible').length-1]);
		}
		GALIBRARY.createWebEventWithLabel('Add Search Field', "Search Field Added", "Search Field Added");
	});
	...
	$(".browseindexlookup").click(function(event) {
	    ...
	    if (((selectedDbMask & NTIS) == NTIS) && ((lookupindex == 'ST') || (lookupindex == 'PN')))
	    {
	        return;
	    }
	    else
	    {
	        var tabloc="/search/browseindex.url?database="+(selectedDbMask)+"&lookup="+(lookupindex)+"&searchtype=Quick";
...
function generateYear(selectedDbMask, sYear, strYear, eYear, searchform)
{
    ...
    if (formstartyear.length == 0 || formendyear.length == 0) return;
    ...
    for(var j=sy; j<=ey; j++)
    {
	    formstartyear.append($('<option value="'+j+(j==dy?'" selected="selected"':'')+'">'+j+'</option>'));
	    formendyear.append($('<option value="'+j+(j==eYear?'" selected="selected"':'')+'">'+j+'</option>'));
    }
...
function searchValidation()
{
    ...
    $( "input.searchword" ).each(function( index ) {

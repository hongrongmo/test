...
<xsl:stylesheet
    ...
    xmlns:actionbean="java:org.ei.stripes.action.results.SearchResultsAbstractAction"
...
<xsl:template match="PAGE">
    ...
    <xsl:variable name="DEDUP-RESULTS-COUNT"><xsl:value-of select="//DEDUP-RESULTS-COUNT"/></xsl:variable>
    ...
    <xsl:value-of select="actionbean:setSearchid($actionbean,$SEARCH-ID)"></xsl:value-of>
    ...
    <!--
    TMH - removed per A&E migration; use A&E fence instead! 
	...
	<xsl:value-of select="actionbean:setUpdatesNo($actionbean,SESSION-DATA/LASTFOURUPDATES)"/>
	<xsl:value-of select="actionbean:setEmailalertweek($actionbean, EMAILALERTWEEK)"/>
	
	<xsl:value-of select="actionbean:setDedupResultCount($actionbean, $DEDUP-RESULTS-COUNT)"/>
	<xsl:value-of select="actionbean:setDisplayPagination($actionbean, //DISPLAY-PAGINATION)"/>

...
public class DDSDatabase {
	...
	private static Log log = LogFactory.getLog("DDSDatabase");
	...
	public DDS findDDS(String contractid) {
	...
	public boolean saveDDS(Options options) {

		boolean result = false;
		DDS dds = (DDS) options;
		try {
			if (findDDS(dds.getContractID()) != null) {
				result = updateDDS(dds);
			} else {
				result = insertDDS(dds);
			}

		} catch (Exception e) {
			log.error("savedds ", e);
		}
		(new U_ACCESS_DDSDatabase()).saveDDS_U_ACCESS(options);
		return result;
	}

	private boolean updateDDS(DDS dds) throws SQLException {
	...
	private boolean insertDDS(DDS dds) {

...
/**
 * Implementation of a Ternary Search Trie, a data structure for storing <code>String</code> objects that combines the compact size of a binary search tree with
 * the speed of a digital search trie, and is therefore ideal for practical use in sorting and searching data.</p>
 * <p>
 * 
 * This data structure is faster than hashing for many typical search problems, and supports a broader range of useful problems and operations. Ternary searches
 * are faster than hashing and more powerful, too.
 * </p>
 * <p>
 * 
 * The theory of ternary search trees was described at a symposium in 1997 (see "Fast Algorithms for Sorting and Searching Strings," by J.L. Bentley and R.
 * Sedgewick, Proceedings of the 8th Annual ACM-SIAM Symposium on Discrete Algorithms, January 1997). Algorithms in C, Third Edition, by Robert Sedgewick
 * (Addison-Wesley, 1998) provides yet another view of ternary search trees.
 * 
...
public class TernarySearchTrie {
    ...
    /**
     * Constructs a Ternary Search Trie and loads data from a <code>File</code> into the Trie. The file is a normal text document, where each line is of the
     * form word : integer.
     * 
     * @param file
    ...
    /**
     * Deletes the node passed in as an argument. If this node has non-null data, then both the node and the data will be deleted. It also deletes any other
     ...
     * 
     * To delete a node, first set its data to null, then pass it into this method, then pass the node returned by this method into this method (make sure you
     * don't delete the data of any of the nodes returned from this method!) and continue in this fashion until the node returned by this method is
     * <code>null</code>.
     * 
     * The TSTNode instance returned by this method will be next node to be operated on by <code>deleteNodeRecursion</code> (This emulates recursive method call
     * while avoiding the JVM overhead normally associated with a recursive method.)
     * 
     * @param currentNode
     *            The node to delete.
    ...
    /**
     * Retrieve the <code>Integer</code> indexed by key, increment it by one unit and store the new <code>Integer</code>.
     * 
     * @param key
     *            A <code>String</code> index.
    ...
    /**
     * Returns the node indexed by key, or <code>null</code> if that node doesn't exist. Search begins at root node.
     * 
     * @param key
     *            A <code>String</code> that indexes the node that is returned.
     * @return The node object indexed by key. This object is an instance of an inner class named <code>TernarySearchTrie.TSTNode</code>.
     */
    ...
    /**
     * Returns a <code>List</code> of keys that almost match the argument key. Keys returned will have exactly diff characters that do not match the target key,
     * where diff is equal to the last value passed in as an argument to the <code>setMatchAlmostDiff</code> method.
     * <p>
     * If the <code>matchAlmost</code> method is called before the <code>setMatchAlmostDiff</code> method has been called for the first time, then diff = 0.
     * 
     * @param key
     *            The target key.
     ...
     * @return A <code>List</code> with the results
     */
    protected List<String> matchAlmost(String key, int numReturnValues) {
     ...
     *            The results so far.
     * @param upTo
     *            If true all keys having up to and including matchAlmostDiff mismatched letters will be included in the result (including a key that is exactly
     *            the same as the target string) otherwise keys will be included in the result only if they have exactly matchAlmostDiff number of mismatched
     *            letters.
     * @param matchAlmostKey
     *            The key being searched.
     * @return A <code>List</code> with the results.
     */
    private List<String> matchAlmostRecursion(TSTNode currentNode, int charIndex, int d, String matchAlmostKey, int matchAlmostNumReturnValues,
        ...
        matchAlmostResult = matchAlmostRecursion(currentNode.relatives[TSTNode.EQKID], charIndex + 1, nextD, matchAlmostKey, matchAlmostNumReturnValues,
            matchAlmostResult, upTo);
        if ((d > 0) || (charComp > 0)) {
    ...
    /**
     * Returns an alphabetical <code>List</code> of all keys in the trie that begin with a given prefix. Only keys for nodes having non-null data are included
     ...
     * 
     * @param key
     *            A <code>string</code> that indexes the object to be removed from the Trie.
     */
    ...
    /**
     * Sets the number of characters by which words can differ from target word when calling the <code>matchAlmost</code> method.
     * <p>
    ...
    /**
     * Sets the default maximum number of values returned from the <code>matchPrefix</code> and <code>matchAlmost</code> methods.
     * <p>
     * The value should be set this to -1 to get an unlimited number of return values. note that the methods mentioned above provide overloaded versions that
     * allow you to specify the maximum number of return values, in which case this value is temporarily overridden.
     * 
     ** @param num
     ...
     * <p>
     * Sorted keys will be appended to the end of the resulting <code>List</code>. The result may be empty when this method is invoked, but may not be
     * <code>null</code>.
     * 
     * @param currentNode
     *            The current node.

...
public class NTISCombiner extends Combiner {
    ...
    private String[] countries = { "France", "Germany", "Netherlands", "Japan", "China", "Russia", "Italy", "Spain", "Canada", "Sweden", "Finland",
        "United Kingdom", "Australia", "Denmark", "International Organizations", "Antigua and Barbuda", "Afghanistan", "Algeria", "Azerbaijan", "Albania",
        "Armenia", "Andorra", "Angola", "American Samoa", "Argentina", "Austria", "Anguilla", "Antarctica", "Bahrain", "Barbados", "Botswana", "Bermuda",
        "Belgium", "Bahamas", "Bangladesh", "Belize", "Bosnia and Herzegovina", "Bolivia", "Burma", "Benin", "Belarus", "Solomon Islands", "Brazil", "Bhutan",
        "Bulgaria", "Brunei Darussalamy", "Burundi", "Cambodia", "Chad", "Sri Lanka", "Congo", "Zaire", "Chile", "Cocos (Keeling) Islands", "Cameroon",
        "Colombia", "Costa Rica", "Central African Republic", "Cuba", "Cape Verde", "Cyprus", "Czechoslovakia", "Djibouti", "Dominica", "Dominican Republic",
        "Ecuador", "Egypt", "Ireland", "Equatorial Guinea", "Estonia", "El Salvador", "Ethiopia", "Czech Republic", "Falkland Islands (Malvinas)",
        "French Guiana ", "Fiji", "Faroe Islands", "Gambia", "Gabon", "	German Democratic Republic ", "Germany Federal Republic", "Ghana", "Gilbraltar ",
        "Grenada", "Greenland", "Guadeloupe", "Guam", "Greece", "Guatemala", "Guinea", "Guyana", "Haiti ", "Hong Kong", "Honduras", "Croatia", "Hungary",
        "Iceland", "Indonesia", "India", "British Indian Ocean Territory", "United States Minor Outer Isl.", "Iran (Islamic Republic of)", "Israel",
        "Ivory Coast", "Iraq", "Jamaica", "Jordan", "Kenya ", "Kyrgyzstan ", "Korea, People's Democratic Rep", "Kiribati", "Korea, Republic of", "Kuwait",
        "Kazakhstan", "Laos, People's Democratic Rep.", "Lebanon", "Latvia", "Lithuania", "Liberia", "Slovakia", "Liechtenstein", "Lesotho", "Luxembourg",
        "Libyan Arab Jamahiriya", "Madagascar", "Martinique", "Macua", "Moldova", "Mogolia", "Montserrat", "Malawi", "Macedonia", "Mali", "Monaco", "Morocco",
        "Mauritius", "Mauritania", "Malta", "Oman", "Maldives", "Montenegro", "Mexico", "Malaysia", "Mozambique", "Netherlands Antilles", "New Caledonia",
        "Niue", "Norfolk Island", "Niger", "Nigeria", "Norway", "Nepal", "Nauru", "Suriname", "Nicaragua", "New Zealand", "Paraguay", "Pitcairn", "Peru",
        "Pakistan", "Poland", "Panama", "Portugal", "Papua New Guinea", "Qutar", "Reunion", "rhodesia", "Romania", "Philippines", "Puerto Rico", "Rwanda",
        "Saudi Arabia", "Seychelles", "South Africa", "Senegal", "St. Helena", "Slovenia", "Sierra Leone", "San Marino", "Singapore", "Somalia", "Serbia",
        "Sudan", "Svalbard and Jan Mayen Islands", "Syrian Arab Republic", "	Switzerland ", "United Arab Emirates", "Trinidad and Tobago", "Thailand",
        "Tajikistan", "Turks and Caicos Islands", "Tokelau", "Tonga", "Togo", "Sao Tome and Principe", "Tunisia", "Turkey", "Tuvalu",
        "Taiwan, Province of China", "Turkmenistan", "Tanzania, United Republic of", "Uganda", "Ukraine", "USSR", "United States", "Burkina FASO", "Uruguay",
        "Uzbekistan", "Saint Vicent & the Grenadines", "Venezuela", "Virgin Islands (British)", "Viet Nam", "Virgin Islands (United States)",
        "Vatican City State (Holy See)", "Namibia", "Wallis and Futuna Islands", "Western Sahara", "Samoa", "Swaziland", "Yemen, Democratic Republic of",
        "Yemen, Republic of", "Yugoslavia", "Zambia", "Zimbabwe", "International Organization" };
    ...
    public static void main(String args[]) throws Exception {
        ...
        CombinedWriter writer = new CombinedXMLWriter(recsPerbatch, loadNumber, "ntis");
    ...
    public void writeCombinedByYearHook(Connection con, int year) throws Exception {
        ...
        try {
            ...
            String q = "select LOAD_NUMBER,M_ID,AN,TI,TN,PN,AB,IC,SU,DES,IDE,SO,PA1,PA2,PA2,PA3,PA4,PA5,RD,RN,CAT,VI,XP,AV,MAA1,MAA2,CT,PR,HN,seq_num from "
                + Combiner.TABLENAME + " where seq_num is not null and substr(load_number,1,4) ='" + year + "'";
    ...
    private void writeRecs(ResultSet rs) throws Exception {
        ...
        while (rs.next()) {
            ...
            String aut = NTISAuthor.formatAuthors(rs.getString("PA1"), rs.getString("PA2"), rs.getString("PA3"), rs.getString("PA4"), rs.getString("PA5"),
                rs.getString("HN"));
            ...
            if (cv != null) {
                rec.put(EVCombinedRec.CONTROLLED_TERMS, prepareMulti(cv));
            }

            String uncontrolled = formatDelimiter(formatFL(rs.getString("IDE")));
            if (uncontrolled != null) {
                rec.put(EVCombinedRec.UNCONTROLLED_TERMS, prepareMulti(uncontrolled));
            }
            ...
            String order = formatDelimiter(formatOrderNumbers(rs.getString("CT"), rs.getString("PN")));
            if (order != null) {
            ...
            if (classcodes != null) {
                rec.put(EVCombinedRec.CLASSIFICATION_CODE, prepareMulti(XMLWriterCommon.formatClassCodes(classcodes)));
            }
            ...
            rec.put(EVCombinedRec.AVAILABILITY, getAvailability(rs.getString("AV"), rs.getString("PR")));
            ...
            rec.put(EVCombinedRec.LANGUAGE, prepareMulti(getLanguage(rs.getString("IC"), rs.getString("SU"))));
            ...
            rec.put(EVCombinedRec.PUB_YEAR, getPubYear(rs.getString("RD"), rs.getString("LOAD_NUMBER")));
            String agency = getAgency(rs.getString("MAA1"), rs.getString("MAA2"));
            if (agency != null) {
                rec.put(EVCombinedRec.AGENCY, prepareMulti(agency));
            }
            rec.put(EVCombinedRec.ACCESSION_NUMBER, formatAccessionNumber(rs.getString("AN")));
    ...
    private String formatOrderNumbers(String num1, String num2) {
    ...
    private String getPubYear(String rd, String load_number) {
        String mResult = null;
        String years19 = "19";
        String years20 = "20";
    ...
    private String getAgency(String maa1, String maa2) {
    ...
    private String getLanguage(String fielddataIC, String fielddataSU) {
        String mResult;
        String language = "English";
        if (fielddataIC != null && fielddataIC.length() > 0) {
    ...
    private String getAvailability(String vn, String pr) {
    ...
    public void writeCombinedByWeekHook(Connection con, int weekNumber) throws Exception {
        ...
        try {
            ...
            stmt = con.createStatement();
            rs = stmt
                .executeQuery("select LOAD_NUMBER,M_ID,AN,TI,TN,PN,AB,IC,SU,DES,IDE,SO,PA1,PA2,PA2,PA3,PA4,PA5,RD,RN,CAT,VI,XP,AV,MAA1,MAA2,CT,PR,HN,seq_num from "

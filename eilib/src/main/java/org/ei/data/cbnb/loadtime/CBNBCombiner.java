...
public class CBNBCombiner extends Combiner {
    ...
    public void writeCombinedByYearHook(Connection con, int year) throws Exception {
        ...
        try {
            ...
            rs = stmt
                .executeQuery("select m_id, abn, doc, sco, fjl, isn, cdn, lan, ibn, src, scc,sct, ebt, cin, vol, iss, pag, reg, cym, sic, gic, gid, atl, otl, abs, edn, SUBSTR(pbn,1,4) pyr,pbn,avl, pbr, load_number, seq_num from "
                    + Combiner.TABLENAME + " where substr(pbn,1,4) ='" + year + "' and seq_num is not null");
    ...
    public void writeCombinedByWeekHook(Connection con, int weekNumber) throws Exception {
        ...
        try {
            ...
            stmt = con.createStatement();
            rs = stmt
                .executeQuery("select m_id, abn, doc, sco, fjl, isn, cdn, lan, ibn, src, scc,sct, ebt, cin, vol, iss, pag, reg, cym, sic, gic, gid, atl, otl, abs, edn, SUBSTR(pbn,1,4) pyr,pbn,avl,pbr,seq_num,load_number from "
                    + Combiner.TABLENAME + " where seq_num is not null and load_number ='" + weekNumber + "'");
            writeRecs(rs);
            this.writer.end();
            this.writer.flush();

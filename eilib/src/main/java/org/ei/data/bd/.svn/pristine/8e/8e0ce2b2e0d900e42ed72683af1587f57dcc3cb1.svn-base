	...
	private List parsePersonalNameString()
	{
		...
		if(personalName != null && personalName.indexOf(BdParser.AUDELIMITER)>0)
		{
			...
			while(personalNameGroupToken.hasMoreTokens())
			{
				...
				if(personalNameGroupString != null && personalNameGroupString.indexOf(BdParser.IDDELIMITER)>0)
				{
					singlePersonalNameObject = personalNameGroupString.split(BdParser.IDDELIMITER);

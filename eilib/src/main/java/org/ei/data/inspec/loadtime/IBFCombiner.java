...
public class IBFCombiner extends Combiner {
    ...
    public static void main(String args[]) throws Exception {
        ...
        CombinedWriter writer = new CombinedXMLWriter(recsPerbatch, loadNumber, "ibf");
    ...
    public void writeCombinedByYearHook(Connection con, int year) throws Exception {
        ...
        try {
            ...
            rs = stmt.executeQuery("select " + QUERY_FIELDS + " from " + Combiner.TABLENAME + " where seq_num is not null and nvl(pyr,substr(su,1,4)) ='"
                + year + "'");
            writeRecs(rs);
    ...
    private void writeRecs(ResultSet rs) throws Exception {
        while (rs.next()) {
            ...
            if (rs.getString("rtype") != null) {
                rec.put(EVCombinedRec.DOCTYPE, getDocType(rs.getString("rtype")));
            }
            ...
            if (rs.getString("pyr") != null) {
                rec.put(EVCombinedRec.PUB_YEAR, getPubYear(rs.getString("pyr")));
            } else {
                rec.put(EVCombinedRec.PUB_YEAR, getPubYear(rs.getString("su").substring(0, 4)));
            }
            ...
            if (rs.getString("cvs") != null || rs.getString("ocvs") != null) {
                ...
                rec.put(EVCombinedRec.CONTROLLED_TERMS, prepareMulti(cvs.toString()));
            }
            ...
            if (rs.getString("ocls") != null) {
                rec.put(EVCombinedRec.INDUSTRIALCODES, prepareMulti(getOrigCLS(rs.getString("ocls"))));
            }

            String dis = getDiscipline(rs.getString("cls"));

            if (dis != null) {
                rec.put(EVCombinedRec.DISCIPLINE, prepareMulti(dis));
            }

            // Source Info
            if (rs.getString("fjt") != null) {
                rec.put(EVCombinedRec.SERIAL_TITLE, rs.getString("fjt"));
            } else if (rs.getString("ojt") != null)
                rec.put(EVCombinedRec.SERIAL_TITLE, rs.getString("ojt"));

            if (rs.getString("fttj") != null) {
                rec.put(EVCombinedRec.SERIAL_TITLE_TRANSLATION, rs.getString("fttj"));

            } else if (rs.getString("ottj") != null) {
                rec.put(EVCombinedRec.SERIAL_TITLE_TRANSLATION, rs.getString("ottj"));

            }

            rec.put(EVCombinedRec.VOLUME, rs.getString("vol"));
            rec.put(EVCombinedRec.ISSUE, rs.getString("iss"));
            rec.put(EVCombinedRec.STARTPAGE, getFirstPage(rs.getString("ipn")));
            ...
            if (rs.getString("source") != null || rs.getString("tsource") != null) {
                ...
                rec.put(EVCombinedRec.SOURCE, prepareMulti(src.toString()));
            ...
            if (rs.getString("thlp") != null) {
                rec.put(EVCombinedRec.MONOGRAPH_TITLE, rs.getString("thlp"));
            }
            ...
            if (rs.getString("cloc") != null || rs.getString("ccnf") != null) {
                ...
                rec.put(EVCombinedRec.CONFERENCE_LOCATION, buf.toString());
            }
            ...
            if (rs.getString("sorg") != null) {
                rec.put(EVCombinedRec.SPONSOR_NAME, rs.getString("sorg"));
            }
            ...
            rec.put(EVCombinedRec.DEDUPKEY, getDedupKey(rec.get(EVCombinedRec.ISSN), rec.get(EVCombinedRec.CODEN), rs.getString("vol"), rs.getString("iss"), rs.getString("ipn")));
    ...
    private String getAdditionalSources(String s) {
        ...
        while (st.hasMoreTokens()) {
            ...
            ArrayList<String> l = getPipeFormat(strToken);
    ...
    private ArrayList<String> getPipeFormat(String strSource) {
        int start = 0;
        int end = 0;

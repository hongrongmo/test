...
public class INSPECCombiner extends Combiner {
    ...
    public static void main(String args[]) throws Exception {
        ...
        CombinedWriter writer = new CombinedXMLWriter(recsPerbatch, loadNumber, INS_DATABASE.getIndexName(), environment);
    ...
    public void writeCombinedByYearHook(Connection con, int year) throws Exception {
        ...
        try {
            ...
            if (year == 2999) {
                rs = stmt
                    .executeQuery("select m_id, fdate, opan, copa, ppdate, sspdate, aaff, afc, ab, anum, pubti, su, pyr, nrtype, pdoi, cdate, cedate, aoi, aus, aus2, rnum, pnum, cpat, ciorg, iorg, pas, chi, pvoliss, pvol, piss, pipn, cloc, cls, pcdn, scdn, cvs, eaff, eds, pfjt, sfjt, fls, pajt, sajt, la, matid, ndi, pspdate, pepdate, popdate, sopdate, ppub, rtype, sbn, sorg, psn, ssn, tc, pubti, ti, trs, trmc, aaffmulti1, aaffmulti2, eaffmulti1, eaffmulti2, nssn, npsn, LOAD_NUMBER, seq_num, ipc from "
                        + Combiner.TABLENAME
                        + " where pyr='0294' or pyr='0994' or pyr='1101' or pyr='20007' or pyr='Dec.' or pyr='July' or pyr is null and seq_num is not null");
            } else {
                rs = stmt
                    .executeQuery("select m_id, fdate, opan, copa, ppdate, sspdate, aaff, afc, ab, anum, pubti, su, pyr, nrtype, pdoi, cdate, cedate, aoi, aus, aus2, rnum, pnum, cpat, ciorg, iorg, pas, chi, pvoliss, pvol, piss, pipn, cloc, cls, pcdn, scdn, cvs, eaff, eds, pfjt, sfjt, fls, pajt, sajt, la, matid, ndi, pspdate, pepdate, popdate, sopdate, ppub, rtype, sbn, sorg, psn, ssn, tc, pubti, ti, trs, trmc, aaffmulti1, aaffmulti2, eaffmulti1, eaffmulti2, nssn, npsn, LOAD_NUMBER, seq_num, ipc from "
                        + Combiner.TABLENAME + " where pyr ='" + year + "' and seq_num is not null");
            }
            writeRecs(rs);
        ...
        while (rs.next()) {
            ...
            if (validYear(strYear)) {
                ...
                if (rs.getString("pubti") != null) {
                    rec.put(EVCombinedRec.MONOGRAPH_TITLE, rs.getString("pubti"));
                ...
                if (rs.getString("trmc") != null) {
                    rec.put(EVCombinedRec.TREATMENT_CODE, prepareMulti(getTreatmentCode(rs.getString("trmc"))));
                }

                if (rs.getString("la") != null) {
                    rec.put(EVCombinedRec.LANGUAGE, prepareMulti(rs.getString("la")));
                }
                ...
                if (rs.getString("tc") != null) {
                    rec.put(EVCombinedRec.CONFERENCE_NAME, rs.getString("tc"));
                } else {
                    String thlp = null;
                    if (rs.getString("pfjt") != null) {
                    ...
                    if (rt1 != null) {
                        if (rt1.startsWith("CA") && thlp != null) {
                            rec.put(EVCombinedRec.CONFERENCE_NAME, thlp);
                        } else if (rt1.startsWith("CP") && rec.get(EVCombinedRec.TITLE) != null) {
                            rec.put(EVCombinedRec.CONFERENCE_NAME, rec.get(EVCombinedRec.TITLE));
                        }
                    }
                }

                if (rs.getString("cloc") != null) {
                    rec.put(EVCombinedRec.CONFERENCE_LOCATION, rs.getString("cloc"));
                }
                ...
                if (rs.getString("sorg") != null) {
                    rec.put(EVCombinedRec.SPONSOR_NAME, prepareMulti(rs.getString("sorg")));
                }
                ...
                if (rs.getString("rnum") != null) {
                    rec.put(EVCombinedRec.REPORTNUMBER, rs.getString("rnum"));
                }

                // Patent Info
                if (rs.getString("pnum") != null) {
                    rec.put(EVCombinedRec.PATENT_NUMBER, rs.getString("pnum"));
                }
                ...
                if (rs.getString("fdate") != null) {
                    rec.put(EVCombinedRec.PATENT_FILING_DATE, rs.getString("fdate"));
                }

                if (rs.getString("opan") != null) {
                    rec.put(EVCombinedRec.APPLICATION_NUMBER, rs.getString("opan"));
                }
                ...
                if (rs.getString("ppdate") != null) {
                    rec.put(EVCombinedRec.PRIORITY_DATE, rs.getString("ppdate"));
                }
                ...
                if (rs.getString("iorg") != null) {
                    rec.put(EVCombinedRec.NOTES, prepareMulti(rs.getString("iorg")));
                ...
                rec.put(EVCombinedRec.DEDUPKEY,
                    getDedupKey(rec.get(EVCombinedRec.ISSN), rec.get(EVCombinedRec.CODEN), rs.getString("pvol"), rs.getString("piss"), rs.getString("pipn")));
    ...
    String[] prepareIndexterms(String aString) throws Exception {

        ArrayList<String> list = new ArrayList<String>();
        aString = perl.substitute("s/" + InspecXMLReader.IDDELIMITER + "/ /g", aString);
    ...
    private String[] getISSN(String nsn) {
        StringTokenizer toks = new StringTokenizer(nsn, InspecXMLReader.AUDELIMITER);
        ArrayList<String> list = new ArrayList<String>();
        String s;
    ...
    private String getTreatmentCode(String tc) {
        StringBuffer buf = new StringBuffer();
        int i = 0;
        StringTokenizer toks = new StringTokenizer(tc, InspecXMLReader.AUDELIMITER);

        while (toks.hasMoreTokens()) {
            String tok = toks.nextToken();
            if (i != 0) {
                buf.append(InspecXMLReader.AUDELIMITER);
            }
    ...
    private String getDiscipline(String dis) {
        ...
        StringTokenizer t = new StringTokenizer(dis, InspecXMLReader.AUDELIMITER);
    ...
    public void writeCombinedByWeekHook(Connection con, int loadN) throws Exception {
        Statement stmt = null;
        ResultSet rs = null;
        ...
        try {
            ...
            // rs =
            // stmt.executeQuery("select m_id, aaff, su, ab, anum, aoi, aus, aus2,pyr, rnum, pnum, cpat, ciorg, iorg, pas, cdate, cedate, doi, nrtype, doit, chi, voliss, ipn, cloc, cls, cn, cnt, cvs, eaff, eds, fjt, fls, fttj, la, matid, ndi, pdate, pub, rtype, sbn, sorg, sn, snt, tc, tdate, thlp, ti, trs, trmc, LOAD_NUMBER from "+Combiner.TABLENAME+
            // " where LOAD_NUMBER = "+loadN);
            if (loadN == 3001) {
                sqlQuery = "select m_id, fdate, opan, copa, ppdate,sspdate, aaff, afc, su, pubti, pfjt, pajt, sfjt, sajt, ab, anum, aoi, aus, aus2, pyr, rnum, pnum, cpat, ciorg, iorg, pas, pcdn, scdn, cdate, cedate, pdoi, nrtype, chi, pvoliss, pvol, piss, pipn, cloc, cls, cvs, eaff, eds, fls, la, matid, ndi, pspdate, ppub, rtype, sbn, sorg, psn, ssn, tc, sspdate, ti, trs, trmc, aaffmulti1, aaffmulti2, eaffmulti1, eaffmulti2, nssn, npsn, LOAD_NUMBER, seq_num, ipc from "
                    + Combiner.TABLENAME + " where pyr is null and load_number < 200537 and seq_num is not null";
            } else if (loadN == 3002) {
                sqlQuery = "select m_id, fdate, opan, copa, ppdate,sspdate, aaff, afc, su, pubti, pfjt, pajt, sfjt, sajt, ab, anum, aoi, aus, aus2, pyr, rnum, pnum, cpat, ciorg, iorg, pas, pcdn, scdn, cdate, cedate, pdoi, nrtype, chi, pvoliss, pvol, piss, pipn, cloc, cls, cvs, eaff, eds, fls, la, matid, ndi, pspdate, ppub, rtype, sbn, sorg, psn, ssn, tc, sspdate, ti, trs, trmc,aaffmulti1, aaffmulti2, eaffmulti1, eaffmulti2, nssn, npsn,  LOAD_NUMBER, seq_num, ipc from "
                    + Combiner.TABLENAME + " where pyr like '194%'or pyr like '195%' or (pyr like '196%' and pyr != '1969') and seq_num is not null";
            } else {
                sqlQuery = "select m_id, fdate, opan, copa, ppdate,sspdate, aaff, afc, su, pubti, pfjt, pajt, sfjt, sajt, ab, anum, aoi, aus, aus2, pyr, rnum, pnum, cpat, ciorg, iorg, pas, pcdn, scdn, cdate, cedate, pdoi, nrtype, chi, pvoliss, pvol, piss, pipn, cloc, cls, cvs, eaff, eds, fls, la, matid, ndi, pspdate, ppub, rtype, sbn, sorg, psn, ssn, tc, sspdate, ti, trs, trmc,aaffmulti1, aaffmulti2, eaffmulti1, eaffmulti2, nssn, npsn, LOAD_NUMBER, seq_num, ipc from "
                    + Combiner.TABLENAME + " where seq_num is not null and LOAD_NUMBER = " + loadN;
            }

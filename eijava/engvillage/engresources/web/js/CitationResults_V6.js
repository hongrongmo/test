    ...
    */
    ...
    {
        var dedup = selectform.DupFlag.value;
        var dbpref = selectform.dbpref.value;
        var fieldpref = selectform.fieldpref.value;
        var dbLink = selectform.dbLink.value;
        var origin = selectform.origin.value;
        var linkSet = selectform.linkSet.value;
        ...
        var  tagSearchFlag = selectform.tagSearchFlag.value;
        var  scope = selectform.scope.value;
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    "&CID=addSelectedRange&searchid="+searchid+"&searchquery="+searchquery+
                    "&searchtype="+searchtype+"&database="+database+"&databaseid="+databaseid+
                    "&startrange="+startrange+"&endrange="+endrange+"&pagesize="+pagesize+
                    "&resultscount="+resultscount+"&count="+count+"&displaytype="+displaytype+
                    "&DupFlag="+dedup+"&dbpref="+dbpref+"&fieldpref="+fieldpref+"&origin="+origin+
                    "&dbLink="+dbLink+"&linkSet="+linkSet+"&tagSearchFlag="+tagSearchFlag+"&scope="+scope;
                }
            } // else
        } // else
    } // function
    ...
    */
    ...
    function selectUnselectAllRecords(selectoption)
    {
        ...
        img.src="/engresources/Basket.jsp?select="+selectoption+"&database="+escape(database)+"&searchquery="+escape(searchquery)+"&sessionid="+sessionid+"&searchid="+escape(searchid)+docidstring+handlestring+"&timestamp=" + milli;
        /*
    ...
    */
    ...
    function clearAllSelectedRecords(selectvalue,sessionid,resultscount)
    {
        ...
        var img = new Image() ;
        var cbsize = 0;
        img.src="/engresources/Basket.jsp?select="+selectvalue+"&sessionid="+sessionid+"&timestamp=" + milli;
        /*
    ...
    */
    ...
    function selectUnselectRecord(thebox,handle,docid,searchquery,database,sessionid,searchid,resultscount)
    {
          ...
          if(thebox.checked)
          {
            ...
            var i = new Image();
            i.src = "/engresources/Basket.jsp?"+
            "select=mark&handle="+handle+"&docid="+docid+"&database="+escape(database)+"&sessionid="+
            sessionid+"&searchquery="+escape(searchquery)+"&searchid="+escape(searchid)+"&timestamp="+ milli;
          } 
          else 
          {
            //window.alert(" deleting from the basket");
            var e = new Image();
            e.src = "/engresources/Basket.jsp?"+
            "select=unmark&handle="+handle+"&docid="+docid+"&database="+escape(database)+
            "&sessionid="+sessionid+"&searchquery="+escape(searchquery)+"&searchid="+escape(searchid)+"&timestamp="+ milli ;
          }
     }

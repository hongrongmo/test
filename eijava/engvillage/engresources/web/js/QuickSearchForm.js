...
function generateYear(selectedDbMask, sYear, strYear, eYear, searchform)
{
    ...
    // null out previous startYear/endYear values
...
function searchValidation()
{
    ...
    if(document.quicksearch.startYear)
    {
        var startYear = null;
        var endYear = null;
        var startYearLength=document.quicksearch.startYear.length;
        var endYearLength=document.quicksearch.endYear.length;

        for(var i=0;i < startYearLength; i++) {
            if(document.quicksearch.startYear[i].selected) {
                startYear=document.quicksearch.startYear[i].value;
                break;
            }
        }

        for(var i=0;i < endYearLength; i++) {
            if(document.quicksearch.endYear[i].selected) {
                endYear=document.quicksearch.endYear[i].value;
                break;
            }
        }

        if(document.quicksearch.yearselect[0].checked)
        {

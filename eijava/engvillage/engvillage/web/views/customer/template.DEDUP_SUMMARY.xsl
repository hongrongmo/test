...
<!--
    <xsl:choose>
      ...
      <xsl:when test="(SEARCH-TYPE='Expert') or (SEARCH-TYPE='Easy')">expertSearchCitationFormat</xsl:when>
...
-->
  ...
  <table border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td colspan="3" height="15"><img src="/engresources/images/s.gif" height="15"/></td>
      </tr>
      <tr>
        ...
        <td valign="top">
            ...
            <xsl:when test="not($REMOVED-DUPS='0')">
                ...
                <xsl:if test="(($LINKSET='removed') and string($DBLINK)) or not($LINKSET='removed')">
                  ...
                  <xsl:attribute name="href">/controller/servlet/Controller?CID=dedup&amp;SEARCHID=<xsl:value-of select="$SEARCH-ID"/>&amp;database=<xsl:value-of select="$SELECTED-DB"/>&amp;dbpref=<xsl:value-of select="$DB-PREF"/>&amp;fieldpref=<xsl:value-of select="$FIELD-PREF"/>&amp;origin=summary&amp;linkSet=removed</xsl:attribute>
                </xsl:if>
            ...
            <xsl:for-each select="DEDUPSET-REMOVED-DUPS/REMOVED-DUPS">
              <a class="SmBlackText">
                <xsl:if test="(($LINKSET='removed') and (not($DBLINK=@DB) and not(@COUNT=0))) or (not($LINKSET='removed') and not(@COUNT=0))">
                  <xsl:attribute name="class">SrtLink</xsl:attribute>
                  <xsl:attribute name="href">/controller/servlet/Controller?CID=dedup&amp;SEARCHID=<xsl:value-of select="$SEARCH-ID"/>&amp;database=<xsl:value-of select="$SELECTED-DB"/>&amp;dbpref=<xsl:value-of select="$DB-PREF"/>&amp;fieldpref=<xsl:value-of select="$FIELD-PREF"/>&amp;dbLink=<xsl:value-of select="@DB"/>&amp;origin=summary&amp;linkSet=removed</xsl:attribute>
                </xsl:if>
        ...
        <td valign="top">
            ...
            <a class="SmBlackText">
                <xsl:if test="(string($LINKSET) and ($LINKSET='deduped') and string($DBLINK)) or (string($LINKSET) and not($LINKSET='deduped'))">
                  <xsl:attribute name="class">SrtLink</xsl:attribute>
                  <xsl:attribute name="href">/controller/servlet/Controller?CID=dedup&amp;SEARCHID=<xsl:value-of select="$SEARCH-ID"/>&amp;database=<xsl:value-of select="$SELECTED-DB"/>&amp;dbpref=<xsl:value-of select="$DB-PREF"/>&amp;fieldpref=<xsl:value-of select="$FIELD-PREF"/>&amp;origin=summary&amp;linkSet=deduped</xsl:attribute>
                </xsl:if>
            ...
            <xsl:for-each select="DEDUPSET-COUNTER/DB-COUNT">
              <a class="SmBlackText">
                <xsl:if test="(($LINKSET='deduped') and (not($DBLINK=@DB)) and not(@COUNT=0)) or (not($LINKSET='deduped') and not(@COUNT=0))">
                  <xsl:attribute name="class">SrtLink</xsl:attribute>
                  <xsl:attribute name="href">/controller/servlet/Controller?CID=dedup&amp;SEARCHID=<xsl:value-of select="$SEARCH-ID"/>&amp;database=<xsl:value-of select="$SELECTED-DB"/>&amp;dbpref=<xsl:value-of select="$DB-PREF"/>&amp;fieldpref=<xsl:value-of select="$FIELD-PREF"/>&amp;dbLink=<xsl:value-of select="@DB"/>&amp;origin=summary&amp;linkSet=deduped</xsl:attribute>
                </xsl:if>
                <xsl:value-of select="@COUNT"/>&#160;<xsl:value-of select="@DBNAME"/>
              </a>
              <xsl:if test="not(position()=last())">,&#160;</xsl:if>

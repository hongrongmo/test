...
<xsl:stylesheet
    ...
    exclude-result-prefixes="schar hlight java html ibfab xsl rfx"
>
    ...
    <xsl:template match="BKYS|AGS|AUS|EDS|IVS|CLS|FLS|CVS|RGIS|DISPS|CTS|OCVS|OCLS|NDI|CHI|AOI|AFS|EFS|PASM|PEXM|PIM">
        <tr>
  ...
  <!-- Controlled/Uncontrolled child node(s) within VALUE under FLS/CVS/AGS -->
    <xsl:template match="BKY|FL|CV|AG|CT|OC|PS|PA|RGI">
      <xsl:call-template name="LINK">
        <xsl:with-param name="TERM"><xsl:value-of select="normalize-space(text())"/></xsl:with-param>
        <xsl:with-param name="FIELD"><xsl:value-of select="name(.)"/></xsl:with-param>
      </xsl:call-template>
            ...
            <xsl:otherwise>
              <xsl:choose>
                <xsl:when test="($FIELD='PNUM')">
                  <xsl:attribute name="href">/controller/servlet/Controller?CID=quickSearchCitationFormat&amp;searchWord1=<xsl:value-of select="$ENCODED-SEARCH-TERM"/>&amp;section1=<xsl:value-of select="$INDEX-FIELD"/>&amp;database=<xsl:value-of select="$DATABASE"/><xsl:value-of select="$START-YEAR-PARAM"/><xsl:value-of select="$END-YEAR-PARAM"/>&amp;yearselect=<xsl:value-of select="$YEARRANGE"/><xsl:value-of select="$DEFAULT-LINK-SORT"/></xsl:attribute>
                </xsl:when>

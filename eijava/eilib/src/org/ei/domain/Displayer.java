...
public class Displayer {
	...
	public static String referexCheckBoxes(String creds, String selectedColls)
	{
		...
		if (creds != null)
		{
			...
			if (pagDatabase != null) {
				...
				if (!sfs.isEmpty())
				{
					...
					while (sfsItr.hasNext())
					{
						String value = (String) sfsItr.next();

						innercheckboxes.append("<input name=\"col\"");
						if ((selectedColls != null) && (selectedColls.indexOf(value) >= 0))
						{
							innercheckboxes.append(" checked=\"checked\" style=\"vertical-align:middle;\" ");
						}
						innercheckboxes.append("type=\"").append(inputType).append("\" id=\"chk").append(value).append("\" value=\"").append(value).append("\" onClick=\"change(\'database\')\"/>");
						innercheckboxes.append("<label class=\"").append(labelClass).append("\" for=\"chk").append(value).append("\">");
						if (!hasCheckboxes)
						{
							innercheckboxes.append("<B>");
						}
						innercheckboxes.append(pagDatabase.getDataDictionary().getClassCodeTitle(value));
						if (!hasCheckboxes)
						{
							innercheckboxes.append("</B>");
						}
						innercheckboxes.append("</label><br/>");
					}
				}
			}
		}
		...
		if (hasCheckboxes) {
			...
			formhtml.append("<img src=\"/engresources/images/s.gif\" width=\"4\"/></td>");
		...
		formhtml.append("<img src=\"/engresources/images/s.gif\" width=\"15\"/></td>");
		...
		if (hasCheckboxes)
		{
			formhtml.append(HelpIcon.getHelpLink());
		}
	...
	public static String toHTML(int userMask, int selectedMask,
			String searchType) {
		...
		if (searchType.equalsIgnoreCase(Query.TYPE_EASY)) {
			...
			if (isWholeDBMask(umask)) {
				...
				alldbcheckbox.append(" style=\"vertical-align:middle;\" value=\"").append(sum).append("\" id=\"allchkbx\" onClick=\"change(\'alldb\');\"/><label class=\"SmBlackText\" for=\"allchkbx\">All</label></a> &nbsp;&nbsp; ");
				// append the 'All' checkbox
				formhtml.append(alldbcheckbox);
				// append the inner checkboxes
				formhtml.append(innercheckboxes);
			...
			if (isWholeDBMask(umask)) {
				...
				formhtml.append("<input type=\"hidden\" name=\"database\" value=\"").append(umask).append("\"/>");
				formhtml.append("<img src=\"/engresources/images/s.gif\" width=\"4\"/>");
				...
				alldbcheckbox.append(" style=\"vertical-align:middle;\" id=\"allchkbx\" value=\"").append(sum).append("\" onClick=\"change(\'alldb\');\"/><label class=\"SmBlackText\" for=\"allchkbx\">All</label></td>");
				// append the 'All' checkbox
				formhtml.append(alldbcheckbox);
				// append the inner checkboxes
				formhtml.append(innercheckboxes);
...
//	.append(dbMasks[x]).append("\" onClick=\"change(\'database\')\";");
	...
	private class DatabaseCheckbox implements Comparable {
...
//		private String onclickevent = "change(\'database\')";
		...
		public String toString()
		{
			...
			dbcheckbox.append("<label class=\"SmBlackText\" for=\"")
				.append(chkid)
				.append("chkbx\">");

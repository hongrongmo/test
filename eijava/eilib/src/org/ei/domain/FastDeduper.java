...
public class FastDeduper
{
	...
	public FastDeduper(BufferedReader in,
					   String fieldPref,
					   String dbPref)
		throws Exception
	{
		Hashtable doiTable = new Hashtable();
		Hashtable dupkeyTable = new Hashtable();
		...
		while((docID = readDocID(in)) != null)
		{
			...
			if(doi != null &&
			   doiTable.containsKey(doi))
			{
				DocID oldID = (DocID)doiTable.get(doi);
				int logicTest = dupLogic(docID,
									  	 oldID,
									  	 fieldPref,
									  	 dbPref);
				if(logicTest == 1)
				...
				else if(logicTest == 0)
				{
			        ...
			        dupkeyTable.containsKey(dupkey))
			{
	...
	private int dupLogic(DocID docID1,
						 DocID docID2,
						 String fieldPref,
						 String dedupDB)
	{
		...
		else
		{
			...
			if( intDedupOption == 0
				...
				|| ((intDmask1 != 0 && ((intDmask1 & intDedupOption) != intDedupOption))
	...
	public DocID readDocID(BufferedReader in)
		throws IOException
	{
		String line = null;
	...
	private void addDups(DocID newID,
						 DocID oldID,
						 int logicTest)
	{
		newID.setIsDup(true);
		...
		List oldDupObjs = oldID.getDupObjs();
		Iterator it = oldDupObjs.iterator();
		while(it.hasNext())
		{
	...
	private DocID parseDocID(String docIdLine,
							 String databaseLine,
							 String dedupKeyLine,
							 String doiLine,
							 String dmaskLine)
	{
		...
		return new DocID(0,
						 docID,
						 dconfig.getDatabase(database.substring(0,3)),
						 dupkey,
						 doi,
						 dmaskInt);
	}

...
import org.ei.query.base.DatabaseMaskGatherer;
import org.ei.query.base.QueryMask;
import org.ei.query.base.EIQueryWriter;
import org.ei.query.base.FieldException;
...
/**
*   Encapsulates search query statistics such as the query phrase, number of records resulted,
*   language in which to search.A Hashtable with the reqired paramaters as key value pairs are
*   input to this class.
*
**/
...
public class Query implements Comparable
{
    ...
    private static final String[] allFields = { "RGI", "PU", "PM", "PE", "VO", "SU", "SP", "DB", "AN", "RN", "AG", "AV", "CT", "CO", "NT", "PA", "PI", "WK", "YR", "ALL", "KY", "AF", "PN", "ST", "MH", "CL", "CN", "TI", "AB", "AU", "CC", "CV", "FL", "SN", "DT", "BN", "TR", "LA", "CF", "DI", "MI", "NI", "CI", "AI",  "OC","BI" , "PEC" , "PUC" , "PAC", "PK" , "PID" , "PCI","PRN","SIC","PEX","PAM","DOI","PFD","BKS","BKT","PD","CR","PC","IP", "IS", "EB", "CM", "GC", "RC", "RO", "SI", "VOM", "CP", "SC", "GD", "IC","CE", "DS", "SD", "SO", "PPA",
    	//EnCompass fields
    ...
    public ResultsState getResultsState() { return resultsstate; }
    public void setResultsState(ResultsState aresultsstate) { resultsstate = aresultsstate; }
    public void clearResultsState() { resultsstate = new ResultsState(); }

    private Refinements queryrefinements = new Refinements();
    public Refinements getRefinements() { return queryrefinements; }
    ...
    /** DeDup set */
    protected List dupSet = new ArrayList();
...
//    }
    ...
    private void buildDisplayQuery()
    {
        ...
        if((this.sSearchType.equals(TYPE_BOOK)  ||
            this.sSearchType.equals(TYPE_QUICK) ||
            this.sSearchType.equals(TYPE_THESAURUS)) && displayLimits.length() > 0)
        {
			if(base.length() > 0)
			{
				base.append(", ");
    ...
    public BooleanQuery getParseTree() throws FieldException, ParseException, SearchException
    {
    ...
    public void compile() throws FieldException, ParseException, SearchException
    {

        /*
        if(((mask & 8) != 8) && ((mask & 16) != 16)) {
            boosters = new RelevanceBooster[1];
            boosters[0] = new FullDocumentBooster();
        }
        */
    ...
    private void buildPhysicalQuery() throws ParseException, FieldException, SearchException
    {
...
//        System.out.println("Phisical query::::::"+physicalQueryBuffer.toString());
        ...
        BooleanQuery queryTree = (BooleanQuery) parser.parse(physicalQueryBuffer.toString());
      ...
      //  DatabaseMaskGatherer truedb = new DatabaseMaskGatherer();
       ...
       int newmask = qm.getQueryMask(queryTree, mask);
        ...
        if (((mask & DatabaseConfig.USPTO_MASK) != DatabaseConfig.USPTO_MASK) && ((mask & DatabaseConfig.CRC_MASK) != DatabaseConfig.CRC_MASK))
        {
            ...
            Vector invalidFields = fv.validateFields(queryTree);
        ...
        if (sAutoStemming.equalsIgnoreCase(ON) &&
            ((mask & DatabaseConfig.USPTO_MASK) != DatabaseConfig.USPTO_MASK) &&
    ...
    private void buildYearRange()
    {
        if((sStartYear == null) || (sEndYear == null))
        {
            ...
            Map startEndYears = databaseConfig.getStartEndYears(credentials, mask);
    ...
    private void buildSearchBase()
    {
        if (sSearchType.equals(TYPE_QUICK) || sSearchType.equals(TYPE_BOOK))
        {
            ...
            if (sSeaPhr1 != null && sSeaPhr1.length() > 0)
            {
                ...
                {
                    exp1Display.append("((").append(sSeaPhr1).append(") WN ").append(sSeaOpt1).append(")");
            ...
            if (exp1Display != null && exp2Display != null && exp3Display != null)
            {
                ...
                physicalBase.append("((" + exp1Physical.toString() + " " + sBool1 + " " + exp2Physical.toString() + ") " + sBool2 + " " + exp3Physical.toString() + ")");
    ...
    */
    private String getSearchPhrase() throws InvalidArgumentException
    {
        ...
        if (xmlPhrase == null)
        {
            throw new InvalidArgumentException("Search Phrase is NULL");
        }
    ...
    */
    ...
    public void setSearchPhrase(String sSeaPhr1, String sSeaOpt1, String sBool1, String sSeaPhr2, String sSeaOpt2, String sBool2, String sSeaPhr3, String sSeaOpt3)
    {
        setSeaPhr1(sSeaPhr1);
        setSeaOpt1(sSeaOpt1);
        setBool1(sBool1);
    ...
    */
    public String getSearchQuery() throws HistoryException
    {
    ...
    * @exception InvalidArgumentException Thrown if the input parameter is null
    */
    ...
    */
    public String getPhysicalQuery() throws HistoryException
    {
    ...
    public void setIntermediateQuery(String querystring)
     {
         if (querystring == null)
         {
             // do nothing
             return;
             //throw new InvalidArgumentException("Intermediate Query should not be NULL");
         }

         this.intermediatequery = querystring;
     }

    public String getIntermediateQuery() throws HistoryException
    {
    ...
    */
    public String toString()
    {
        ...
        return (new StringBuffer().append(sPhrase).append(getRecordCount()).append(getLanguage()).append(getStartYear()).append(getEndYear()).append(getDataBase()).append(getAutoStemming()).append(getSortOption()).append(getSearchType()).append(getID()).append(getDisplayQuery())
        //.append(getPhysicalQuery())
        .append(getDocumentType()).append(getTreatmentType()).append(getDisciplineType()).append(getLastFourUpdates()).append(getDupSet().toString()).append(getDeDupDB()).append(isDeDupString()).toString());
    }
    ...
    */
     ...
     *
     * @param Object An object of this class type
    * @return int Zero or less than zero or greater than zero on comparizen(?) comparison
    ...
    // Do not report Runtime exception subclass! ==> throws ClassCastException
    {
        ...
        if (o == null)
        {
            // from Sun Javadoc "Note that null is not an instance of any class, and e.compareTo(null) should throw a NullPointerException
            // even though e.equals(null) returns false."
            throw new NullPointerException("null is not an instance of Query object");
        }

        // Original Implementation - if query ids are equal, then compareTo returns equal
        Query que = (Query) o;
        ...
        try {
            result =
                ((this.getSearchPhrase()).equals(que.getSearchPhrase())
                &&
                (this.getLanguage()).equals(que.getLanguage())
                &&
                (this.getStartYear()).equals(que.getStartYear())
                &&
                (this.getEndYear()).equals(que.getEndYear())
                &&
                (this.getDataBase() == que.getDataBase())
                &&
                (this.getAutoStemming()).equals(que.getAutoStemming())
                &&
                ...
                &&
                (this.getSearchType()).equals(que.getSearchType())
                &&
                (this.getDocumentType()).equals(que.getDocumentType())
                &&
                (this.getTreatmentType()).equals(que.getTreatmentType())
                &&
                (this.getDisciplineType()).equals(que.getDisciplineType()) ? 0 : -1);
        }
    ...
    /** Overrides Object.
    *
    * @param Object reference of QueryData

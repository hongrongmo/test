	...
	throws Exception
{
	...
	try
	{
		...
		writerLang	= new PrintWriter(new FileWriter(load_number_begin+dbname+"_lang.lkp"));

		if(load_number_end == 0)
		{
			pstmt1	= con.prepareStatement(" select language_text from "+dbname+"_master where (language_text is not null) and load_number = "+load_number_begin);
			System.out.println("\n\nQuery: "+" select language_text from "+dbname+"_master where (language_text is not null) and load_number = "+load_number_begin);
		}
		else
		{
			pstmt1	= con.prepareStatement(" select language_text from "+dbname+"_master where (language_text is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
			System.out.println("\n\nQuery: "+" select language_text from "+dbname+"_master where (language_text is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
		}

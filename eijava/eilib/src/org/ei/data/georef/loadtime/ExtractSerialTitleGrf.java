...
public class ExtractSerialTitleGrf{
	...
	public static void main(String[] args) throws Exception
	{
		...
		est.extract(0, 0, con, "georef_test");
	}
	...
	throws Exception
{
	...
	try
	{
		...
		{
			pstmt1	= con.prepareStatement(" select title_of_serial from "+dbname+" where (title_of_serial is not null)");
			System.out.println("\n\nQuery: "+" select title_of_serial from "+dbname+" where (title_of_serial is not null)");
		}
		else
		{
			pstmt1	= con.prepareStatement(" select title_of_serial from "+dbname+" where (title_of_serial is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
			System.out.println("\n\nQuery: "+" select title_of_serial from "+dbname+" where (title_of_serial is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
		}

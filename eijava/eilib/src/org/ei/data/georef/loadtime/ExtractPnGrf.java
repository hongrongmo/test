	...
	throws Exception
{
	...
	try
	{
		...
		{
			pstmt1	= con.prepareStatement(" select publisher from "+dbname+"_master where (publisher is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
			System.out.println("\n\nQuery: "+" select publisher from "+dbname+"_master where (publisher is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
		}

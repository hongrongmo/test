    ...
    */
    ...
    private List loadDocument(List listOfDocIDs, String dataFormat) throws Exception
	{
		...
		try
        {
			...
			System.out.println("QUERY::"+queryBD+INString);
	...
	private String  getStartPage(String page)
	{
		...
		System.out.println("START PAGE "+strPage.trim());
		return strPage.trim();
	}
	...
	private String  getEndPage(String page)
	{
		...
		System.out.println("END PAGE "+strPage.trim());
		return strPage.trim();
	}
	...
	private String getPageCount(String pageCount)
	{
		if(pageCount != null)
		{
			String[] pagesCountElement = setElementData(pageCount);
			StringBuffer pagesBuffer = new StringBuffer();
			for(int i=0;i<pagesCountElement.length;i++)
			{
				String[] pagesCount = pagesCountElement[i].split(BdParser.IDDELIMITER,-1);
				if(pagesCount.length>1 && pagesCount[1] != null && pagesCount[1].length()>0)
				{
					if(pagesBuffer.length()>0)
					{
						pagesBuffer.append(",");
					}
					pagesBuffer.append(pagesCount[1]);
				}

			}
			if(pagesBuffer.length()>0)
			{
				return pagesBuffer.toString();
			}
			return null;
		}
		return null;

	...
	extends Combiner
{
    	...
    	throws Exception
    {
        ...
        try
        {
            ...
            rs = stmt.executeQuery("select abs, m_id, cna, chn, tie, aut, adr, sti, SUBSTR(pyr,1,4) pyr, cod, isn, lna, ctm, crd, trc, vol,iss,pag, std, tif, cfl,to_char(to_date(spd),'yyyymmdd') pub_sort, pub, doi , load_number from " + Combiner.TABLENAME + " where SUBSTR(pyr, 1, 4) ='"+year+"'");
            ...
            if (validYear(rs.getString("pyr")))
            {
                ...
                if (rs.getString("lna") != null)
                {
                    rec.put(EVCombinedRec.LANGUAGE,
                            prepareMulti(rs.getString("lna")));
                }

                if (rs.getString("ctm") != null)
                {
                    rec.put(EVCombinedRec.CONTROLLED_TERMS,
                            prepareMulti(rs.getString("ctm")));
                }

                if (rs.getString("crd") != null)
                {
                    rec.put(EVCombinedRec.CASREGISTRYNUMBER,
                            prepareMulti(rs.getString("crd")));
                }

                if (rs.getString("trc") != null)
                {
                    rec.put(EVCombinedRec.CLASSIFICATION_CODE,
                            prepareMulti(XMLWriterCommon.formatClassCodes(rs.getString("trc"))));
                }
...
//  12/04/2007  TS added spec on it : if there is no comma in the adr field - this is a redundant au name - remove it
    	...
    	throws Exception
    {
        ...
        try
        {
            ...
            rs = stmt.executeQuery("select abs, m_id, cna, chn, tie, aut, adr, sti, SUBSTR(pyr,1,4) pyr, cod, isn, lna, ctm, crd, trc, vol,iss,pag, std, tif, cfl,to_char(to_date(spd),'yyyymmdd') pub_sort, pub, doi, load_number from " + Combiner.TABLENAME + " where load_number =" + weekNumber);

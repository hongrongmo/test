		...
		throws Exception
	{
		...
		try
		{
			...
			if(load_number_end == 0)
			{
				...
				System.out.println("\n\nQuery: "+" select authors from "+dbname+"_master where (authors is not null) and load_number = "+load_number_begin);
			}
			else
			{
				pstmt1	= con.prepareStatement(" select authors from "+dbname+"_master where (authors is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
				System.out.println("\n\nQuery: "+" select authors from "+dbname+"_master where (authors is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
			}

...
public class ExtractCvsGeo
{
		...
		throws Exception
	{
		...
		try
		{
			...
			{
				pstmt1	= con.prepareStatement(" select descriptor_mainterm_gde from "+dbname+"_master where (descriptor_mainterm_gde is not null) and load_number = "+load_number_begin+yearString);
				System.out.println("\n\nQuery: "+" select descriptor_mainterm_gde from "+dbname+"_master where (descriptor_mainterm_gde is not null) and load_number = "+load_number_begin+yearString);
			}
			else
			{
				pstmt1	= con.prepareStatement(" select descriptor_mainterm_gde from "+dbname+"_master where (descriptor_mainterm_gde is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end+yearString);
				System.out.println("\n\nQuery: "+" select descriptor_mainterm_gde from "+dbname+"_master where (descriptor_mainterm_gde is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end+yearString);
			}

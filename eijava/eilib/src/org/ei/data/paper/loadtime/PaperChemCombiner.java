...
public class PaperChemCombiner
	extends Combiner
{
    ...
    public void writeCombinedByYearHook(Connection con, int year)
    	throws Exception
    {
        ...
        try
        {
            ...
            if (year != 3000)
            {
				rs = stmt.executeQuery("select M_ID, id, an, ex, ig, su, ab, au, cp, em, af, ac, asp, ay, ed, ef, ec, es, ey, mt, st, se, cf, bn, sn, cn, vo, iss, sd, SUBSTR(yr,1,4) yr, md, m1, m2, ml, mc, ms, my, sp, pa, pn, pc, ps, py, pp, xp, vx, la, dt, cc, at, nr, ti, tt, tr, cls, fl, pt, pi, do, rn, media, csess, patno, pling, appln, prior_num, assig, pcode, claim, sourc, nofig, notab, sub_index, specn, suppl, pdfix, LOAD_NUMBER from " + Combiner.TABLENAME + " where SUBSTR(yr,1,4) ='"+year+"' AND load_number < 1000000" );
			}
            else
            {
				rs = stmt.executeQuery("select M_ID, id, an, ex, ig, su, ab, au, cp, em, af, ac, asp, ay, ed, ef, ec, es, ey, mt, st, se, cf, bn, sn, cn, vo, iss, sd, SUBSTR(yr,1,4) yr, md, m1, m2, ml, mc, ms, my, sp, pa, pn, pc, ps, py, pp, xp, vx, la, dt, cc, at, nr, ti, tt, tr, cls, fl, pt, pi, do, rn, media, csess, patno, pling, appln, prior_num, assig, pcode, claim, sourc, nofig, notab, sub_index, specn, suppl, pdfix, LOAD_NUMBER from " + Combiner.TABLENAME + " where "+where);
			}
    	...
    	throws Exception
    {
        ...
        while (rs.next())
        {
            ...
            if (validYear(rs.getString("yr")))
            {
                ...
                {
                    rec.put(EVCombinedRec.COMPANIES,
                            prepareMulti(rs.getString("assig")));
                }
                ...
                String patinfo = formatPatentInfo(rs.getString("appln"),
                        						  rs.getString("prior_num"),
                        						  rs.getString("pling"),
                        						  rs.getString("pcode"));
    ...
    private String formatPatentInfo(String app,
            						String prn,
            						String pln,
            						String pcd)
    {

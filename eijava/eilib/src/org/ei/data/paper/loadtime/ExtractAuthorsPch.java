...
public class ExtractAuthorsPch
{
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            writerAuthor    = new PrintWriter(new FileWriter("pch_aus.lkp"));

            if(load_number_end == 0)
            {
                pstmt1  = con.prepareStatement(" select au from paper_master where (au is not null) and load_number = "+load_number_begin);
                System.out.println("\n\nQuery: "+" select au from paper_master where (au is not null) and load_number = "+load_number_begin);
            }
            else
            {
                pstmt1  = con.prepareStatement(" select au from paper_master where (au is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
                System.out.println("\n\nQuery: "+" select au from paper_master where (au is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
            }
            ...
            for(int i = 0; itrTest.hasNext(); i++)
            {
                ...
                writerAuthor.println(display_name+"\t"+au_index_name+"\tpch");
            }

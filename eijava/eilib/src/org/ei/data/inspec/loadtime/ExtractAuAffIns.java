...
public class ExtractAuAffIns
{
		...
		throws Exception
	{
		...
		try
		{
			...
			{
				pstmt1	= con.prepareStatement(" select aaff from new_ins_master where (aaff is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
				System.out.println("\n\nQuery: "+" select aaff from new_ins_master where (aaff is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
			}

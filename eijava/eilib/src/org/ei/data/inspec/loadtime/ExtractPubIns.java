...
public class ExtractPubIns
{
		...
		throws Exception
	{
		...
		try
		{
			...
			{
				pstmt1	= con.prepareStatement(" select ppub from new_ins_master where (ppub is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
				System.out.println("\n\nQuery: "+" select ppub from new_ins_master where (ppub is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
			}

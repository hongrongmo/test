...
public class CPXDocument
{
	...
	public Hashtable getHighlightData(String field)
	{
...
//   Accessor and Mutators methods
	...
	private String getStartPage()
	{
	 ...
	 {
		 StringBuffer sb = new StringBuffer();
		 if(editorAffiCountry != null &&
		    editorAffiCountry.length() > 0)
		 {
		 	sb.append("<"+ eiEditorAffiCountryTag+"><![CDATA["+editorAffiCountry+"]]></"+ eiEditorAffiCountryTag+">");
	 	 }

	 	 if(editorAssoState != null &&
	 	    editorAssoState.length() > 0)
	 	 {
	...
	public String getPublisher()
	{
		...
		if(publisherOrderNum != null &&
		   publisherOrderNum.length() > 0)
		{
		...
		if((publisherCity != null && publisherCity.length() > 0)||
		   (publisherState != null && publisherState.length() > 0) ||
		   (publisherProvince != null && publisherProvince.length() > 0) ||
		   (publisherCountry != null && publisherCountry.length() > 0))
		{
	...
	public String getConference()
	{
		...
		if(conferenceCode != null &&
		   conferenceCode.length() > 0)
		{
			sb.append("<"+eiConferenceCodeTag+"><![CDATA["+conferenceCode+"]]></"+eiConferenceCodeTag+">");
		}

		if((conferenceCity != null && conferenceCity.length() > 0) ||
		   (conferenceState != null && conferenceState.length() > 0) ||
		   (conferenceProvince != null && conferenceProvince.length() > 0) ||
		   (conferenceCountry != null && conferenceCountry.length() > 0))
		{
    ...
    public String getSponsor()
    {
		...
		if((sponsorCity != null && sponsorCity.length() > 0) ||
		   (sponsorState != null && sponsorState.length() > 0) ||
		   (sponsorProvince != null && sponsorProvince.length() > 0) ||
		   (sponsorCountry != null && sponsorCountry.length() > 0))
		{
  ...
  */
    		...
    		throws IOException
   {
		   ...
		   authors.length() > 0)
		{
			...
			{
				xmlAuthStr.append("<").append(eiAuthorTag).append(">").append("<![CDATA[").append(author).append("]]>").append("</").append(eiAuthorTag).append(">");
		    }
		 }
	...
	private String getAuthorForLocalHolding(String Authors){
 		...
 		throws IOException
 	{
		   ...
		   editors.length() > 0)
		{
			...
			{
				xmlEditStr.append("<").append(eiEditorTag).append(">").append("<![CDATA[").append(editor).append("]]>").append("</").append(eiEditorTag).append(">");
		    }
		 }
	...
	**/
	...
	/** This method returns Uncontrolled Terms.
	 ...
	 private String getUnControlledTerms(String unConTerms)
	 {
		...
		private String includeCDataXML(String methodResult,
									   String methodTag)
		{

			if((methodResult==null)||
			   (methodResult.trim().equals("")))
			{
				return "";
			}
			else

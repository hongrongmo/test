...
public class CompendexRecordFixer
{
		...
		throws IOException
	{
		...
		String fieldName = CompendexBaseTableRecord.baseTableFields[index];
		...
		else if(fieldName.equals("AUS"))
		{
			if(data.length() > AUS_MAXSIZE)
			{
				data = fixAuthors(data);
			}

		}
		else if(fieldName.equals("VT"))
		{
			if(data.length() > VT_MAXSIZE)
			{
				data = "QQ";
			}
		}
		...
		else if(fieldName.equals("LF"))
		{
			if(data.length() > LF_MAXSIZE)
			{
				data = "Z";
			}
		}
	...
	private String getFirst(String data,
							int maxSize)
	{
		String first = "";
		...
		throws IOException
	{
		...
		{
			if((buf.length()+author.length()) < AUS_MAXSIZE)
			{
				if(buf.length() > 0)
				{
					buf.append(";");

...
public class ExtractStCpx
{
		...
		throws Exception
	{
		...
		try
		{
			...
			{
				pstmt1	= con.prepareStatement(" select st,sn from "+dbname+"_master where (st is not null) and load_number = "+load_number_begin);
				System.out.println("\n\nQuery: "+" select st,sn from "+dbname+"_master where (st is not null) and load_number = "+load_number_begin);
			}
			else
			{
				pstmt1	= con.prepareStatement(" select st,sn from "+dbname+"_master where (st is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
				System.out.println("\n\nQuery: "+" select st,sn from "+dbname+"_master where (st is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
			}
			...
			for(int i = 0; itrTest.hasNext(); i++)
			{
				...
				writerSt.println(issn.trim()+"\t"+source_title+"\t"+dbname.toLowerCase());
			}

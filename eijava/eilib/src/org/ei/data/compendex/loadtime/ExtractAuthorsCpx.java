...
public class ExtractAuthorsCpx
{
		...
		throws Exception
	{
		...
		try
		{
			...
			{
				pstmt1	= con.prepareStatement(" select aus from "+dbname+"_master where (aus is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
				System.out.println("\n\nQuery: "+" select aus from "+dbname+"_master where (aus is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
			}

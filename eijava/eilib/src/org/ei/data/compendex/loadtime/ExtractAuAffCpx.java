...
public class ExtractAuAffCpx
{
		...
		throws Exception
	{
		...
		try
		{
			...
			{
				pstmt1	= con.prepareStatement(" select af from "+dbname+"_master where (af is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
				System.out.println("\n\nQuery: "+" select af from "+dbname+"_master where (af is not null) and load_number >= "+load_number_begin+" and load_number <= "+load_number_end);
			}

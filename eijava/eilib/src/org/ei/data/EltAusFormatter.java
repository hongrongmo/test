...
public class EltAusFormatter {
    ...
    public static String formatAffiliation(String aaf) {
        ...
        for (int i = 0; i < arrAaf.length; i++) {
            ...
            if (c.charValue() != '|' && !c.isDigit(arrAaf[i]))
    ...
    public String formatAuthors(String sAuthors) {
        ...
        List lstAuthors = new ArrayList();
        List lstObjAuthors = new ArrayList();

        StringTokenizer tokens = new StringTokenizer(sAuthors, "|", false);

        while (tokens.hasMoreTokens()) {
            List lstIndexes = new ArrayList();
        ...
        for (Iterator iter = lstObjAuthors.iterator(); iter.hasNext();) {
        ...
        for (int i = 0; i < arrAaf.length; i++) {
            ...
            if (c.charValue() != '|' && c.charValue() != ':' && !c.isDigit(arrAaf[i]))

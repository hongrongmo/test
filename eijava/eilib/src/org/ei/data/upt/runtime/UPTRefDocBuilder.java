    ...
    DocumentBuilder
{
    ...
    public List buildNonPatentRefs(String parentID)
        throws DocumentBuilderException
    {
        List list = null;
    ...
    private List loadNonPatentRefs(String parentID)
        throws Exception
    {
    ...
    private List loadCitations(List lstOfDocIDs,
    ...
    /*
    public List loadPatentRefs(List docIDs)
        throws Exception
    {
        ArrayList buildFromBaseTable = new ArrayList();
        ArrayList buildFromRefsTable = new ArrayList();

        for(int i=0; i<docIDs.size(); i++)
        {

            DocID docID = (DocID)docIDs.get(i);
            String did = docID.getDocID();

            ElementDataMap ht = new ElementDataMap();

            if (did.indexOf("upt") == 0)
            {
                buildFromBaseTable.add(docID);
            }
            else if(did.indexOf("ref") == 0)
            {
                buildFromRefsTable.add(docID);
            }
        }

        ArrayList finishedList = new ArrayList();

        if(buildFromBaseTable.size() > 0)
        {
            List fromBaseTable = buildFromBaseTable(buildFromBaseTable);
            finishedList.addAll(fromBaseTable);
        }

        if(buildFromRefsTable.size() > 0)
        {
            List fromRefsTable = buildFromRefsTable(buildFromRefsTable);
            finishedList.addAll(fromRefsTable);
        }
    ...
    public List buildFromBaseTable(List docIDs)
        throws Exception
    {
        DatabaseConfig config = DatabaseConfig.getInstance();
        Database d = config.getDatabase("upt");
        DocumentBuilder patentBuilder = d.newBuilderInstance();
        return patentBuilder.buildPage(docIDs, Citation.CITATION_FORMAT);
    }

    private List buildFromRefsTable(List docIDs)
        throws Exception
    {
        return loadCitations(docIDs);
    }

    class EIDocComparator implements Comparator
    {
        public int compare(Object o1, Object o2)
        {
            EIDoc a1 = (EIDoc) o1;
            EIDoc a2 = (EIDoc) o2;
            int index1 = a1.getDocID().getHitIndex();
            int index2 = a2.getDocID().getHitIndex();
    ...
    */
    ...
    public RefPager getRefPager(String parentID,
                                int pageSize,
                                String sessionID)
        throws Exception
    {
        ...
        RefPager refPager = new RefPager(linked,
                                         pageSize,
                                         refQuery,
                                         sessionID);
        return refPager;
    ...
    private List getUnLinkedRefIDs(String parentID)
                    throws Exception
    {
        List list = new ArrayList();
        int count = 0;
    ...
    private List getLinkedRefIDs(String parentID,
                                 PatRefQuery query)
            throws Exception
    {
        List list = new ArrayList();

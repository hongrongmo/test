...
public class NonPatRefsMatcher
{
    ...
    public static void main(String[] args)throws Exception
    {
        ...
        //String sql="SELECT  rtrim(trim(REPLACE(REPLACE(ref_raw,'&amp;#x201c;','\"'),'&amp;#x201d;','\"')),',') as ref_raw,doi,cpx,ins,cpx as c84,ins as ibf  FROM NON_PAT_REFS WHERE ref_raw  LIKE '% et al., &amp;#x201c;%' AND  ref_raw like '%IEEE%' and load_number IN(71,72)";
        //String sql="SELECT ref_raw, doi,cpx,ins,c84,ibf From non_pat_refs_new where load_number=72 and  ref_raw LIKE '%&amp;#x201c%' and  LOWER(ref_raw) not  LIKE '%brochure%' and LOWER(ref_raw) not LIKE '%patent%' and LOWER(ref_raw) not LIKE '%www.%'and LOWER(ref_raw) not LIKE 'u.s. appl. no.%' and cpx is null and ins is null and ibf is null and c84 is null";
        ...
        m.getRecords(m.getConnection(args[0],args[1],args[2]),sql);
    ...
    public void getRecords(Connection conn, String sql)throws IOException
    {
        ...
        try {
            ...
            while(rec.next())
            {
                ...
                {
                  Hashtable m= matchFast(buildFastQuery(r));
                  pwLog.println(r);
    ...
    public Hashtable matchFast(String fastQuery)throws Exception
    {
    ...
    public String buildFastQuery(NonPatRef ref)
    {  //(au:"Smith" AND ti:water AND vo:2 AND su:6 AND yr:2004 AND sp:935)  AND (((db:cpx OR db:c84))) -->

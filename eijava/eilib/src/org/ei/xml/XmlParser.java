...
public class XmlParser{
	...
	/** This Method returns a Vector of Child Nodes
	...
	public List getNodes()
	{
		...
		List listOfNodes = Collections.synchronizedList(new ArrayList());
	...
	/** This method returns a node by matching attr value
	  * @param   matchAttr --name of attribute that is to be matched
	  * @param  listOfNodes --list of nodes
	  * @return Node
	  */
	public Node getNodeByMatchingAttr(String matchAttr,List listOfNodes)
	{
		for(int j=0;j<listOfNodes.size();j++)
		{
			Object obj=listOfNodes.get(j);
			Node node=(Node)obj;
	...
	/** This method returns a vector of subnodes of a node.
	 * The node which you got from getNodes method.
	 * @param  node 
	 * @return Vector
	 */
	public List getSubNodes(Node node)
	{
		List listOfSubNodes = Collections.synchronizedList(new ArrayList());
		try
		{
	...
	/**  This method returns Node value from the Node.
	  *  If Node is <test>ravi</test> then the return of this method is "ravi".
	  *  If you don't have value for a particular node then return space.
	  *  @param  node
	  *  @return  String
	  */

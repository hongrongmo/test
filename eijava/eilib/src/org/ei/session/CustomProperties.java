...
public class CustomProperties {
    ...
    /** Read the local holding information for the curent customer
        from the configuration tables and store it in the localHolding
        property of the session. The property is formated as follows:
        link elements are seperated by commas and link groups are
        seperated by vertical bars null fields are left as a single space
        Example:
        linkLabel,dynamicURL,defaultURL,futureURL|linkLabel,dynamicURL,defaultURL,futureURL ...
    */
    ...
    private void readLocalHolding() throws SessionException {
        ...
        try
        {
            if ((user.getCustomerID() != "-") && (user.getContractID() != "0")) {
                sqlQuery = " SELECT LINK_LABEL , DYNAMIC_URL , DEFAULT_URL , FUTURE_URL "
                           +" FROM LOCAL_LINKING_OPTIONS WHERE CUSTOMER_ID = "
                           +user.getCustomerID()
                           +" and CONTRACT_ID = "
                           +user.getContractID();
                ...
                userSes.setProperty(UserSession.LOCAL_HOLDING_KEY,propString.toString());
            }
        } catch(Exception e) {

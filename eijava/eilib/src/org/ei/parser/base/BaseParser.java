...
public class BaseParser extends BaseNodeVisitor
{
    ...
    public BaseNode parse(String searchString)
        throws ParseException
    {
        ...
        try
        {
            ...
            while(e == null && (nextToken = getNextToken()) != null && state != FINISH)
            {
            ...
            {
                throw new ParseException(new Exception("Parse Exception"));
            }
        }
    ...
    private void reduce(Stack stack)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if(ch == '"')
                {
                    ...
                    p = new Term(elit);
                    //System.out.println("Reducing ExactTerm:"+ p.getValue());
                }
            }
        }
        lastToken = p;
    ...
    public void visitWith(BooleanQuery topNode)
    {
        if(stack.size() > 2)
        {
            ...
            if(node3.getType().equals(BooleanQuery.TYPE) ||
               node3.getType().equals(BooleanPhrase.TYPE))
            {
                ...
                if(node3.getType().equals(BooleanPhrase.TYPE))
                {
                    ...
                    //System.out.println("Reducing BooleanQuery:"+ bq.getValue());
                }
                else
                {
                ...
                {
                    AndQuery aquery = new AndQuery((BooleanQuery)bq,
                                       (BooleanAnd)node2,
                                       topNode);
                    stack.pop(); stack.pop(); stack.pop();
                    stack.push(aquery);
                    //System.out.println("Reduced AndQuery:"+ aquery.getValue());
                    reduce(stack);
                }
                else if(node2.getType().equals(BooleanOr.TYPE))
                {
                    OrQuery oquery = new OrQuery((BooleanQuery)bq,
                                     (BooleanOr)node2,
                                     topNode);
                    stack.pop(); stack.pop(); stack.pop();
                    stack.push(oquery);
                    //System.out.println("Reduced OrQuery:"+ oquery.getValue());
                    reduce(stack);
                }
                else if(node2.getType().equals(BooleanNot.TYPE))
                {
                    NotQuery nquery = new NotQuery((BooleanQuery)bq,
                                     (BooleanNot)node2,
                                     topNode);
                    stack.pop(); stack.pop(); stack.pop();
                    stack.push(nquery);
    ...
    public void visitWith(AndQuery topNode)
    {
        ...
        stack.pop();
        stack.push(bquery);
        //System.out.println("Reduced AndQuery to BooleanQuery:"+ bquery.getValue());
        reduce(stack);
    }
    ...
    public void visitWith(OrQuery topNode)
    {
        ...
        stack.pop();
        stack.push(bquery);
        //System.out.println("Reduced OrQuery to BooleanQuery:"+ bquery.getValue());
        reduce(stack);
    }
    ...
    public void visitWith(NotQuery topNode)
    {
        ...
        stack.pop();
        stack.push(bquery);
        //System.out.println("Reduced NotQuery to BooleanQuery:"+ bquery.getValue());
        reduce(stack);
    }
    ...
    public void visitWith(AndPhrase topNode)
    {
        ...
        stack.pop();
        stack.push(bphrase);
        //System.out.println("Reduced AndPhrase to BooleanPhrase:"+ bphrase.getValue());
        reduce(stack);
    }
    ...
    public void visitWith(OrPhrase topNode)
    {
        ...
        stack.pop();
        stack.push(bphrase);
        //System.out.println("Reduced OrPhrase to BooleanPhrase:"+ bphrase.getValue());
        reduce(stack);
    }
    ...
    public void visitWith(NotPhrase topNode)
    {
        ...
        stack.pop();
        stack.push(bphrase);
        //System.out.println("Reduced NotPhrase to BooleanPhrase:"+ bphrase.getValue());
        reduce(stack);
    }
    ...
    public void visitWith(Field topNode)
    {
        ...
        if(stack.size() >= 3 && node2.getType().equals(KeywordWITHIN.TYPE))
        {
            ...
            if(node3.getType().equals(BooleanPhrase.TYPE))
            {
                Expression ex = new Expression((BooleanPhrase)node3,
                                               (KeywordWITHIN)node2,
                                            topNode);
                    stack.pop(); stack.pop(); stack.pop();
                    stack.push(ex);
                    //System.out.println("Reducing BooleanPhrase to Expression:"+ ex.getValue());
                    reduce(stack);
            }
            else
            {
    ...
    public void visitWith(CloseParen topNode)
    {
        ...
        if(node2.getType().equals(BooleanQuery.TYPE) &&
           node3.getType().equals(OpenParen.TYPE))
        {

            BooleanQuery q = new BooleanQuery((OpenParen)node3,
                                              (BooleanQuery)node2,
                                      topNode);
            //System.out.println("Reducing BooleanQuery inside Parens:"+ q.getValue());
            stack.pop(); stack.pop(); stack.pop();
            stack.push(q);
            reduce(stack);
        }
        else if(node2.getType().equals(BooleanPhrase.TYPE) &&
                node3.getType().equals(OpenParen.TYPE))
        {

            BooleanPhrase q = new BooleanPhrase((OpenParen)node3,
                                                (BooleanPhrase)node2,
                                        topNode);
                //System.out.println("Reducing BooleanPhrase inside Parens:"+ q.getValue());
            stack.pop(); stack.pop(); stack.pop();
            stack.push(q);
            reduce(stack);
        }
        ...
        if(stack.size() > 2)
        {
            ...
            if(node2.getType().equals(ProximityOperator.TYPE))
            {
                if(node3.getType().equals(Term.TYPE))
                {
                    ...
                    stack.pop();stack.pop();stack.pop();
                    stack.push(pphrase);
                    //System.out.println("Reducing ProximityPhrase:"+ pphrase.getValue());
                    reduce(stack);
                }
            }
            else
    ...
    public void visitWith(ProximityPhrase topNode)
    {
        ...
        if(stack.size() > 2)
        {
            ...
            {
                 Phrase p = new Phrase(topNode);
                //System.out.println("Reducing ProximityPhrase to Phrase:" +p.getValue());
                stack.pop();
                stack.push(p);
                reduce(stack);
            }
    ...
    public void visitWith(Expression topNode)
    {
        ...
        stack.pop();
        stack.push(bq);
        //System.out.println("Reducing Expression to BooleanQuery:"+ bq.getValue());
        reduce(stack);
    }
    ...
    public void visitWith(BooleanPhrase topNode)
    {
        ...
        if(stack.size() > 2)
        {
            ...
            if(node3.getType().equals(BooleanPhrase.TYPE))
            {
                if(node2.getType().equals(BooleanAnd.TYPE))
                {
                    ...
                    stack.pop();stack.pop();stack.pop();
                    stack.push(aphrase);
                    //System.out.println("Reducing AndPhrase:"+ aphrase.getValue());
                    reduce(stack);
                }
                else if(node2.getType().equals(BooleanOr.TYPE))
                {
                    ...
                    stack.pop();stack.pop(); stack.pop();
                    stack.push(ophrase);
                    //System.out.println("Reducing OrPhrase:"+ ophrase.getValue());
                    reduce(stack);
                }
                else if(node2.getType().equals(BooleanNot.TYPE))
                {
                    NotPhrase nphrase = new NotPhrase((BooleanPhrase)node3,
                                                      (BooleanNot)node2,
                                      topNode);
                    stack.pop();stack.pop(); stack.pop();
                    //System.out.println("Reducing NotPhrase:"+ nphrase.getValue());
                    stack.push(nphrase);
                    reduce(stack);
                }
            }
            ...
            {
                BooleanQuery bquery = new BooleanQuery(topNode);
                //System.out.println("Reducing BooleanQuery from BooleanPhrase Joel:"+ bquery.getValue());
                stack.pop();
                stack.push(bquery);
                reduce(stack);
            }
        ...
        else
        {
            ...
            if(parenCount == 0)
            {
                ...
                stack.pop();
                stack.push(bquery);
                //System.out.println("Reducing BooleanQuery from BooleanPhrase 2:"+ bquery.getValue());
                reduce(stack);
            }
        }
    }

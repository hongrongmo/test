...
public class TestUtils {
    ...
    public static <T> T parse(String resource, Class<T> clazz)
        ...
        try {
        	...
        	{
				/*
				 * String json = IOUtils.toString(stream); System.out.println("SNS event: " +
				 * json);
				 */
        		return snsEventMapper.readValue(stream, clazz);
        	}
			/*
			 * if (clazz == S3Event.class) { String json = IOUtils.toString(stream);
			 * S3EventNotification event = S3EventNotification.parseJson(json);
			 * 
			 * @SuppressWarnings("unchecked") T result = (T) new
			 * S3Event(event.getRecords()); return result;
			 * 
			 * } else if (clazz == SNSEvent.class) { return snsEventMapper.readValue(stream,
			 * clazz); }

...
<xsl:template match="HEADER">
  ...
  <ttl>2880</ttl>
  <copyright>Copyright <xsl:value-of select="date:year($now)"/> Elsevier Inc. All rights reserved.</copyright>
</xsl:template>
...
<xsl:template match="EI-DOCUMENT">
  <item>
    ...
    <xsl:text disable-output-escaping="yes"><![CDATA[<link>]]></xsl:text>
      ...
      <xsl:text>/search/results/expert.url?</xsl:text>
      ...
      <xsl:text>&amp;endYear=</xsl:text><xsl:value-of select="date:year($now)"/>

             ...
             <tr>
               <td align="right" valign="middle" height="24">
                 <xsl:if test="string($PREV-PAGE)">
                   <a><xsl:attribute name="href">/search/results/tags.url?CID=tagSearch&amp;SEARCHID=<xsl:value-of select="$ENCODED-SEARCH-ID"/>&amp;COUNT=<xsl:value-of select="$PREV-PAGE"/>&amp;scope=<xsl:value-of select="$SCOPE"/>&amp;database=<xsl:value-of select="$DATABASE"/></xsl:attribute>
                   <img src="/static/images/pp.gif" border="0" />
                   </a>
                 </xsl:if>
                 <img width="8" height="1" src="/static/images/s.gif" border="0" />
                 <xsl:if test="string($NEXT-PAGE)">
                   <a><xsl:attribute name="href">/search/results/tags.url?CID=tagSearch&amp;SEARCHID=<xsl:value-of select="$ENCODED-SEARCH-ID"/>&amp;COUNT=<xsl:value-of select="$NEXT-PAGE"/>&amp;scope=<xsl:value-of select="$SCOPE"/>&amp;database=<xsl:value-of select="$DATABASE"/></xsl:attribute>

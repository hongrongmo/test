...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="DATABASE-ID">
    ...
    <html>
      <head>
        <title>
          <xsl:value-of select="$SEARCH-TYPE" /> Search References Format</title>
        ...
        <xsl:text disable-output-escaping="yes">
          <![CDATA[
            ...
            function selectedRangeRefs(_docid, _backurl, _resultscount)
            {
              ...
              else if((startrange >4000) || (endrange >4000))
	      {
              ...
              else
              {
                ...
                else
                {
                  ...
                  else
                  {
                    ...
                    //alert("/controller/servlet/Controller?CID=addSelectedRangeRefs&backurl="+escape(backurl)+"&docid="+docid+"&resultscount="+resultscount+"&startrange="+startrange+"&endrange="+endrange);
            ...
            <tr>
              <td valign="top">
                ...
                <p align="right">
                  ...
                  <a href="#">
                    <img src="/static/images/top.gif" border="0" />
                  </a>
                </p>
              </td>
      ...
      <tr>
        <td align="left">
          ...
          <xsl:if test="($REF-CNT) and not($REF-CNT ='') and not($REF-CNT ='0')">
            <A CLASS="MedBlackText">&#160; - &#160;</A>
            <xsl:choose>
              <xsl:when test="$DATABASE-ID='ins'">
            	<a title="Show records that are referenced by this record" class="BrownText">Inspec Refs</a>&nbsp;<a class="MedBlackText">(<xsl:value-of select="$REF-CNT" />)</a>
              </xsl:when>
              <xsl:otherwise>
		...
		<xsl:value-of select="custoptions:checkFullText($FULLTEXT, $FULLTEXT-LINK, $CUST-ID, EI-DOCUMENT/DO, EI-DOCUMENT/DOC/DB/DBMASK)" />
           ...
           <xsl:if test="($CHECK-CUSTOM-OPT ='true')">
            ...
            <a title="Full-text" onclick="window.open('/controller/servlet/Controller?CID=FullTextLink&amp;docID={$DOC-ID}','newwindow','width=500,height=500,toolbar=no,location=no,scrollbars,resizable');return false">
              <img id="ftimg" src="/static/images/av.gif" alt="Full-text"/></a>
          </xsl:if>
    ...
    <table border="0" cellspacing="0" cellpadding="0" width="99%">
      <tr>
	   ...
	   <xsl:when test="$DATABASE='2'">	   
	   	<a class="BlueText"><b>There <xsl:value-of select="$AREIS" /> <xsl:value-of select="$REF-CNT"/> inspec <xsl:value-of select="$REFS" /> for the following record:</b></a>
	   </xsl:when>
	   <xsl:otherwise>
      ...
      <tr><td><img src="/static/images/s.gif" height="20" /></td></tr>
      <tr>
      ...
      <tr>
        <td valign="top" align="left">
          <a href="javascript:emailFormat('$SESSIONID','{$SEARCH-TYPE}','{$SEARCH-ID}',{$CURRENT-PAGE},{$RESULTS-COUNT},'{$DATABASE}','{$DATABASE-ID}','{$CIT-CNT}')">
            <img src="/static/images/em.gif" border="0" />
          </a>&#160; &#160;
          <a href="javascript:printFormat('$SESSIONID','{$DATABASE}')">
            <img src="/static/images/pr.gif" border="0" />
          </a>&#160; &#160;
          <a href="javascript:downloadFormat('$SESSIONID','{$DATABASE}')">
            <img src="/static/images/dw.gif" border="0" />
          </a>
          <xsl:if test="($PERSONALIZATION-PRESENT='true')">
            ...
            <A>
              ...
              <img src="/static/images/sv.gif" border="0" />
            </A>
          </xsl:if>
          <!-- <img src="/static/images/s.gif" height="10" /> -->
          <a CLASS="MedBlackText">&#160; - &#160;</a>
          ...
          <a href="javascript:selectedRangeRefs('{$ROOT-DOCID}','{$RANGE-BACK-URL}','{$REF-CNT}')"><img align="top" src="/static/images/go.gif" border="0" /></a>
          <a CLASS="MedBlackText">&#160; - &#160;</a>
  ...
  <xsl:template match="REF-DOCS">
    ...
    <xsl:variable name="PREVREF-NAV">
     ...
     <xsl:variable name="REFCOUNT">
           <xsl:choose>
           <xsl:when test="(/PAGE/PAGE-RESULTS/UPT-DOC/REF-COUNT='-1') or (/PAGE/PAGE-RESULTS/UPT-DOC/REF-COUNT &gt; /PAGE/PAGE-RESULTS/UPT-DOC/EI-DOCUMENT/RCT)">
              <xsl:value-of select="/PAGE/PAGE-RESULTS/UPT-DOC/EI-DOCUMENT/RCT" />
           </xsl:when>
           <xsl:otherwise>
           	<xsl:value-of select="/PAGE/PAGE-RESULTS/UPT-DOC/REF-COUNT" />
           </xsl:otherwise>
           </xsl:choose>
     </xsl:variable>
    ...
    <tr>
      <td align="right">
        ...
        <xsl:if test="(@OFFSET &gt; 0)">
          ...
          <img src="/static/images/pp.gif" border="0" /></a>
          &spcr8;
        </xsl:if>
        <xsl:if test="($REFCOUNT &gt; (@OFFSET + $PAGESIZE))">
          <a href="/controller/servlet/Controller?{$NEXTREF-NAV}">
          <img src="/static/images/np.gif" border="0" /></a>
        </xsl:if>
      </td>
    </tr>
  </xsl:template>
...
<!-- ZY: 9/15/09 Append Kind to searchWord1 for EP patents -->
    ...
    <tr>
      <td valign="top" align="left" width="3">
        ...
        <xsl:if test="not((substring($DOC-ID,0,8)) ='ref_ins')">
      ...
      <td valign="top" width="3">
        <img src="/static/images/s.gif" width="3" />
      </td>
      <td valign="top">
        <img src="/static/images/s.gif" name="image_basket" />
        <xsl:apply-templates select="EI-DOCUMENT" />
        <br/>
        <xsl:choose>
          <xsl:when test="not((substring($DOC-ID,0,4)) ='ref')">
            ...
            <xsl:if test="($REF-CNT) and not($REF-CNT ='0') and not($REF-CNT ='')">
              ...
              <xsl:choose>
		<xsl:when test="$DATABASE-ID='ins'">
			<A title="Show articles that are referenced by this record" CLASS="LgBlueLink" HREF="/controller/servlet/Controller?{$REFERENCES-LINK-CID}&amp;DOCINDEX={$INDEX}&amp;PAGEINDEX={$CURRENT-PAGE}&amp;database={$SELECTED-DB}&amp;format=patentReferencesFormat&amp;docid={$DOC-ID}&amp;searchnav={$ENCODED-RESULTS-NAV}">Inspec Refs</A>&nbsp;<A CLASS="MedBlackText">(<xsl:value-of select="$REF-CNT" />)</A>
		</xsl:when>
		<xsl:otherwise>
            ...
            <xsl:if test="($CIT-CNT) and not($CIT-CNT ='0') and not($CIT-CNT ='')">
              <A CLASS="MedBlackText">&#160; - &#160;</A>
              <A title="Show articles that reference this record" CLASS="LgBlueLink" HREF="/controller/servlet/Controller?CID={$CID-PREFIX}CitationFormat&amp;{$REF-CITEDBY-QSTR}&amp;yearselect=yearrange&amp;searchtype={$SEARCH-TYPE}&amp;sort=yr">Cited by</A>&nbsp;<A CLASS="MedBlackText">(<xsl:value-of select="$CIT-CNT" />)</A>
            </xsl:if>
          </xsl:when>
        </xsl:choose>
        <xsl:variable name="CHECK-CUSTOM-OPT">
        ...
        <xsl:if test="($CHECK-CUSTOM-OPT ='true') and not((substring($DOC-ID,0,4)) ='ref')">
          ...
          <a href="" onclick="window.open('/controller/servlet/Controller?CID=FullTextLink&amp;docID={$DOC-ID}','newwindow','width=500,height=500,toolbar=no,location=no,scrollbars,resizable');return false">
          	<img src="/static/images/av.gif" border="0" align="absbottom"/>
        </a></xsl:if>
  ...
  <xsl:template match="NAVIGATORS">
   ...
   <xsl:variable name="REFCTX">
    ...
    <form name="navigator" method="POST">
      <xsl:attribute name="action">/search/results/expert.url?CID=expertSearchCitationFormat&amp;database=<xsl:value-of select="/PAGE/DBMASK" />&amp;RERUN=<xsl:value-of select="$SEARCH-ID" />&amp;resultsorall=all&amp;COUNT=<xsl:value-of select="//PAGE/CURR-PAGE-ID"/></xsl:attribute>
      <!-- preserve sort data -->
      ...
      <table border="0" cellspacing="0" cellpadding="0" width="100%">
        ...
        <tr>
          <td width="1" bgcolor="#3173b5">
            <img src="/static/images/s.gif" border="0" height="3" width="1" />
          </td>
          <td valign="top">
            <img src="/static/images/s.gif" border="0" height="3" width="1" />
          </td>
        ...
        <xsl:if test="not(/PAGE/SESSION-DATA/SEARCH-TYPE='Easy')">
          <tr>
            ...
            <td valign="top" align="right">
              <input onclick="javascript:return navigatorsOnsubmit('limit');" type="image" src="/static/images/sch.gif" align="absbottom" name="search" value="Search" border="0" />&#160;&#160;
              </td>
            <td width="1" bgcolor="#3173b5">
              <img src="/static/images/s.gif" border="0" width="1" />
            </td>
        ...
        <xsl:if test="not(/PAGE/SESSION-DATA/SEARCH-TYPE='Easy')">
                ...
                <tr>
                  <td>
                    <table border="0" width="100%" cellspacing="0" cellpadding="0">
                      <tr>
                        <td colspan="3">
                          <img src="/static/images/s.gif" height="2" />
                        </td>
                      </tr>
                      <tr>
          ...
          <tr>
            <td width="1" bgcolor="#3173b5">
              <img src="/static/images/s.gif" border="0" height="2" width="1" />
            </td>
            <td valign="top">
              <img src="/static/images/s.gif" border="0" height="2" width="1" />
            </td>
          ...
          <tr>
            ...
            <td valign="middle" align="right">
              <input type="image" onclick="javascript:return navigatorsOnsubmit('limit');" src="/static/images/sch.gif" name="search" align="absbottom" value="Search" border="0" />&#160;&#160;
            </td>
            <td width="1" bgcolor="#3173b5">
              <img src="/static/images/s.gif" border="0" width="1" />
            </td>
        ...
        <tr>
          <td width="1" bgcolor="#3173b5">
            <img src="/static/images/s.gif" border="0" height="4" width="1" />
          </td>
          <td valign="top">
            <img src="/static/images/s.gif" border="0" height="4" width="1" />
          </td>
        ...
        <tr>
          <td width="1" bgcolor="#3173b5">
            <img src="/static/images/s.gif" border="0" height="5" width="1" />
          </td>
          <td valign="top" align="right">
            <a CLASS="SmBlueText">
              <b>?</b>
        ...
        <tr>
          <td colspan="3" height="1" bgcolor="#3173b5">
            <img src="/static/images/s.gif" height="1" border="0" />
          </td>
        </tr>
      </table>
    </form>
  ...
  <xsl:template match="MORE">
    ...
    <img src="/static/images/s.gif" width="5" height="1" />
  </xsl:template>

      ...
      <A CLASS="SpLink">
        <xsl:if test="string($SEARCH-TYPE)">
          <xsl:choose>
            ...
            <xsl:otherwise>
              <xsl:attribute name="href">/search/results/quick.url?CID=quickSearchCitationFormat&amp;searchWord1={<xsl:value-of select="$ENCODED-SEARCH-TERM"/>}&amp;section1=<xsl:value-of select="$FIELD"/>&amp;database=<xsl:value-of select="$ENCODED-DATABASE"/>&amp;yearselect=<xsl:value-of select="$YEARRANGE"/>&amp;<xsl:value-of select="$DEFAULT-LINK-SORT"/></xsl:attribute>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:if>

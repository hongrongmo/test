          ...
          <tr>
            <td align="left" valign="middle" height="24">
              <xsl:if test="(string($RESULTS-NAV-URL))">
              	&spcr8;
                <a><xsl:attribute name="href"><xsl:value-of select="$RESULTS-NAV-URL"/></xsl:attribute>
                <img height="16" src="/static/images/sr.gif" border="0" /></a>
              </xsl:if>
              <xsl:if test="(string($DEDUPRESULTS-NAV-URL))">
                &spcr8;
                <a><xsl:attribute name="href">/search/results/dedup.url?<xsl:value-of select="$DEDUPRESULTS-NAV-URL"/></xsl:attribute>
                <img height="16" src="/static/images/dr.gif" border="0" /></a>
              </xsl:if>
              &spcr8;
              <xsl:if test="(string($NEWSEARCH-URL))">
                ...
                <img height="16" src="/static/images/ns.gif" border="0" /></a>
              </xsl:if>
            </td>

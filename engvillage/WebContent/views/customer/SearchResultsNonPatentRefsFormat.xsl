...
<!-- ZY 9/15/09 Append patent kind in searchword1 for EP patents -->
    <xsl:variable name="CITEDBY-QSTR">
      <xsl:choose>
        <xsl:when test="($SEARCH-TYPE='Quick')">/search/results/quick.url?CID=quickSearchCitationFormat&amp;searchWord1=<xsl:value-of select="$CITEDBY-PM"/><xsl:if test="$AUTH-CODE='EP'"><xsl:value-of select="$KIND-CODE"/></xsl:if>&amp;section1=PCI&amp;database=49152&amp;pcinav=0~<xsl:value-of select="$CITEDBY-PM"/>~<xsl:value-of select="java:encode('Patents that cite ')"/><xsl:value-of select="$CITEDBY-PM"/></xsl:when>
        <xsl:otherwise>/search/results/expert.url?CID=expertSearchCitationFormat&amp;searchWord1=<xsl:value-of select="$CITEDBY-PM"/><xsl:if test="$AUTH-CODE='EP'"><xsl:value-of select="$KIND-CODE"/></xsl:if><xsl:value-of select="java:encode(' WN PCI')"/>&amp;database=49152&amp;pcinav=0~<xsl:value-of select="$CITEDBY-PM"/>~<xsl:value-of select="java:encode('Patents that cite ')"/><xsl:value-of select="$CITEDBY-PM"/></xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
...
-->
    ...
    <table border="0" cellspacing="0" cellpadding="0" width="99%">
      ...
      <tr>
		...
		<xsl:when test="$DATABASE='2'">
              		<A title="Show articles that are referenced by this record" CLASS="LgBlueLink" HREF="/controller/servlet/Controller?{$PATENT-REFS-LINK}">Inspec Refs</A>&nbsp;<A CLASS="MedBlackText">(<xsl:value-of select="$REF-CNT"/>)</A>
              	</xsl:when>
              	<xsl:otherwise>
  ...
  <center>
    <table border="0" cellspacing="0" cellpadding="0" width="99%">
      ...
      <tr><td valign="top"><img src="/static/images/s.gif" height="20"/></td></tr>
      <tr>
        <td valign="top" align="left">
          <xsl:apply-templates select="EI-DOCUMENT"/>
        </td>
      </tr>
      <tr><td height="10" colspan="4"><img src="/static/images/s.gif" height="10"/></td></tr>
...
<xsl:template match="PAGE-ENTRY">
  ...
  <tr>
    ...
    <td valign="top" width="3"><img src="/static/images/s.gif" width="3"/></td>
    <td valign="top"><img src="/static/images/s.gif" name="image_basket"/>
      <table>
        <tr>

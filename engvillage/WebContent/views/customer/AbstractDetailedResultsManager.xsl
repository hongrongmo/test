...
<!-- ZY 9/15/09 Append patent kind in searchword1 for EP patents -->
    <xsl:variable name="CITEDBY-QSTR">
      <xsl:choose>
        <xsl:when test="($SEARCH-TYPE='Quick')">/search/results/quick.url?CID=quickSearchCitationFormat&amp;searchWord1=<xsl:value-of select="$CITEDBY-PM"/><xsl:if test="$AUTH-CODE='EP'"><xsl:value-of select="$KIND"/></xsl:if>&amp;section1=PCI&amp;database=49152&amp;pcinav=0~<xsl:value-of select="$CITEDBY-PM"/>~<xsl:value-of select="java:encode('Patents that cite ')"/><xsl:value-of select="$CITEDBY-PM"/></xsl:when>
        <xsl:otherwise>/search/results/expert.url?CID=expertSearchCitationFormat&amp;searchWord1=<xsl:value-of select="$CITEDBY-PM"/><xsl:if test="$AUTH-CODE='EP'"><xsl:value-of select="$KIND"/></xsl:if><xsl:value-of select="java:encode(' WN PCI')"/>&amp;database=49152&amp;pcinav=0~<xsl:value-of select="$CITEDBY-PM"/>~<xsl:value-of select="java:encode('Patents that cite ')"/><xsl:value-of select="$CITEDBY-PM"/></xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
      ...
      <table border="0" cellspacing="0" cellpadding="0" width="99%" >
        ...
        <tr>
          <td width="50%" align="left">
            ...
            <xsl:choose>
		<xsl:when test="(/PAGE/PAGE-RESULTS/PAGE-ENTRY/EI-DOCUMENT/DOC/DB/DBMASK = '2')">
		<xsl:if test="($INS-REF-CNT) and not($INS-REF-CNT ='0') and not($INS-REF-CNT ='')">
			...
			<A title="Show records that are referenced this record" class="LgBlueLink" HREF="/controller/servlet/Controller?{$PATENT-REFS-LINK}&amp;DOCINDEX={$INDEX}&amp;PAGEINDEX={$CURRENT-PAGE}&amp;database={$SELECTED-DB}&amp;format=patentReferencesFormat&amp;docid={$DOC-ID}">Inspec Refs </A><A class="MedBlackText">(<xsl:value-of select="$INS-REF-CNT"/>)</A>
		</xsl:if>
		</xsl:when>
                <xsl:otherwise>
              ...
              <xsl:if test="($CIT-CNT) and not($CIT-CNT ='0') and not($CIT-CNT ='')">
                <A class="MedBlackText">&#160; - &#160;</A>
                <a title="Show patents that reference this patent" class="LgBlueLink" href="{$CITEDBY-QSTR}&amp;yearselect=yearrange&amp;searchtype={$SEARCH-TYPE}&amp;sort=yr">Cited by</a>&#160;<a class="MedBlackText">(<xsl:value-of select="$CIT-CNT" />)</a>
              </xsl:if>
          ...
          <td width="50%" align="right">
            ...
            <a ttile="Save to Folder">
              ...
              </xsl:attribute><img style="vertical-align: middle;" src="/static/images/sv.gif" border="0" /></a>
          </td>
        </tr>
        <tr>
          <td colspan="2">

      ...
      <table border="0" width="99%" cellspacing="0" cellpadding="0" bgcolor="#C3C8D1" >
        <tr>
          <td align="left" valign="middle" height="24">
              ...
              &spcr8;
              <a><xsl:attribute name="href">/search/results/tags.url?CID=tagSearch&amp;searchtype=TagSearch&amp;searchtags=<xsl:value-of select="$SEARCH-ID"/>&amp;COUNT=<xsl:value-of select="$PAGE-INDEX"/>&amp;SCOPE=<xsl:value-of select="$SCOPE"/>&amp;groupID=<xsl:value-of select="$GROUP-ID"/>&amp;database=<xsl:value-of select="$DEFAULT-DB-MASK"/></xsl:attribute>
              <img height="16" src="/static/images/sr.gif" border="0" /></a>
              </xsl:if>
            &spcr8;
            <a><xsl:attribute name="href">/controller/servlet/Controller?CID=<xsl:value-of select="$NEWSEARCHCID"/>&amp;database=<xsl:value-of select="$DEFAULT-DB-MASK"/></xsl:attribute>
              <img height="16" src="/static/images/ns.gif" border="0" />
            </a>
          </td>
          <xsl:choose>

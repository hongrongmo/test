<%@ page language="java" %><%@ page session="false" %><%@ page import="java.util.*"%><%@ page import="org.ei.books.*"%><%@ page import="java.net.URLEncoder"%><%@ page import="org.ei.tags.*"%><%@ page import="org.engvillage.biz.controller.UserSession"%>
<%@ page import="org.ei.domain.personalization.*"%><%@ page import="org.engvillage.biz.controller.ControllerClient"%><%@ page import="org.ei.email.*"%><%@ page import="org.ei.domain.*"%><%@ page errorPage="/error/errorPage.jsp"%><%@ page import="javax.mail.internet.*"%><%
	...
	StringBuffer srurl = getsrURL(index,tagSearch,scopeParam,database);
	...
	StringBuffer editurl = geteditURL(index,tagSearch,scopeParam,database);
	...
	request.setAttribute("RESULTS-COUNT", Integer.toString(nTotalDocs));
...
%><%!
	...
	StringBuffer getsrURL(int recnum,
						  ...
						  String database)
	{
		recnum = firstDocOnPage(recnum);
		StringBuffer srurlbuf = new StringBuffer();
		srurlbuf.append("CID=tagSearch").append("&");
		srurlbuf.append("SEARCHID=").append(URLEncoder.encode(tagSearch)).append("&");
		srurlbuf.append("searchtype=TagSearch").append("&");
	...
	private StringBuffer geteditURL(int index,
									String tagSearch,
									String scopeParam,
									String database)
	{
		StringBuffer addurlbuf = new StringBuffer();
		StringBuffer editConfirmBuf = new StringBuffer();
		editConfirmBuf.append("CID=tagEditConfirm").append("&");
		editConfirmBuf.append("searchtags=").append(URLEncoder.encode(tagSearch)).append("&");
		editConfirmBuf.append("database=").append(database).append("&");
		editConfirmBuf.append("scope=").append(scopeParam);
		addurlbuf.append("CID=").append("editForward").append("&amp;");
		addurlbuf.append("RETURNURL=").append(URLEncoder.encode(editConfirmBuf.toString()));
		return addurlbuf;
	}

	StringBuffer getbookdetURL(int index,
						   	   String tagSearch,
						       String scopeParam,
						       String database,
						       String pii,
						       DocID docID)
	{
		...
		bookdeturlbuf.append("CID=tagSearchBookSummary&amp;");
		bookdeturlbuf.append("SEARCHID=").append(URLEncoder.encode(tagSearch)).append("&amp;");

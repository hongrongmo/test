...
<%
 ...
 try{
	...
	isFullTextPresent = clientCustomizer
	.checkFullText("citationResults");
	isRssLinkPresent = clientCustomizer.checkRssLink();
	isCitLocalHoldingsPresent = clientCustomizer
	.checkLocalHolding("citationResults");
	...
	if (searchTags != null) {
		...
		nTotalDocs = tagBroker.count(searchTags, scopeint, pUserId,
		customerId, groupID, credentials);
		if (nTotalDocs > 0) {
	oPage = tagBroker.getPage(searchTags, scopeint, pUserId,
			customerId, groupID, index, pagesize, sessionId,
			Citation.CITATION_FORMAT, credentials);
	...
	String backURL = URLEncoder.encode(getBackURL(index, searchTags,
	scope, database));
	...
	out.write("<LOCALHOLDINGS-CITATION>" + isCitLocalHoldingsPresent
	+ "</LOCALHOLDINGS-CITATION>");
	out.write("<EMAILALERTS-PRESENT>" + isEmailAlertsPresent
	+ "</EMAILALERTS-PRESENT>");
	out.write("<PERSONALIZATION-PRESENT>" + isPersonalizationPresent
	+ "</PERSONALIZATION-PRESENT>");
	out.write("<PERSONALIZATION>" + personalization
	+ "</PERSONALIZATION>");
	...
	out.write("<CLEAR-ON-VALUE>" + documentBasket.getClearOnNewSearch()
	+ "</CLEAR-ON-VALUE>");
...
<%!private String getnsURL(String database) {
	...
	private String getBackURL(int count, String searchTags, String scope,
			String database) {
	...
	private void pageNavXML(Writer out, int recnum, int totalcount)
			throws Exception {
		int pageIndex = -1;
		if ((recnum % 25) == 0) {
			pageIndex = (recnum / 25);

...
<%
	...
	// delcare variable to hold user agreement flag.
	String sUserAgreement = null;
	...
	// variable to hold confirm password
	String sConfirmPassword = null;
	...
	else
	{
    	...
    	if(request.getParameter("useragreement") != null)
    	{
    		sUserAgreement = request.getParameter("useragreement");
    	}
    	else
    	{
    		sUserAgreement = "false";
    	}
		...
		{
			sConfirmPassword = request.getParameter("confirmpassword").trim();
		}

		boolean createprofile = true;
		if (GenericValidator.isBlankOrNull(sEmail)) createprofile = false;
		else if (GenericValidator.isBlankOrNull(sFirstName)) createprofile = false;
		else if (GenericValidator.isBlankOrNull(sLastName)) createprofile = false;
		else if (GenericValidator.isBlankOrNull(sPassword) || sPassword.length() < 6 || sPassword.length() > 16) createprofile = false;
		else if (!sPassword.equals(sConfirmPassword)) createprofile = false;
		else if ("false".equals(sUserAgreement)) createprofile = false;
		
		PersonalAccount pAccount = new PersonalAccount();
		// first check for email existence.
		boolean emailFlag = false; 
		if (!GenericValidator.isBlankOrNull(sEmail)) {
			emailFlag = pAccount.emailExists(sEmail,customerId);
		}
		// if the email id is not exist create the account.
		if(createprofile && !emailFlag)
		{
			...
			if( nUserId != null )
			{
				...
				ussession.getUser().setUserId(nUserId);
        		//ussession.getUser().setFirstName(userProfile.getFirstName());
        		//ussession.getUser().setLastName(userProfile.getLastName());
			...
			emailText.append("Email address : "+sEmail+"\n\n");
			emailText.append("With your Personal Account, you can save searches, save records, and create Email Alerts—messages sent ");
			...
			emailText.append("(http://www.engineeringvillage.com), simply login using the email address and password that ");
		...
		else
		{
			...
			sb.append("<USER-AGREEMENT>"+sUserAgreement+"</USER-AGREEMENT>");
			sb.append("<PASSWORD>"+sPassword+"</PASSWORD>");
			sb.append("<CONFIRM-PASSWORD>"+sPassword+"</CONFIRM-PASSWORD>");

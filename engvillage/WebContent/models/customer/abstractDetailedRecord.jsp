...
<%@ page import="java.io.*"%>
<%-- <%@ page import="org.ei.data.upt.runtime.UPTRefDocBuilder"%>
<%@ page import="org.ei.data.upt.runtime.UPTRefDatabase"%> --%>
...
<%
    ...
    try
    {
        ...
        String displayPagination = request.getParameter("displayPagination");

        if(request.getParameter("dedupResultCount")!=null)
        {
        	dedupResultCount=request.getParameter("dedupResultCount").trim();
        }
        ...
        else
        {
            ...
            if(sDocId!=null && !sDocId.equals("") && !sDocId.equals("null")){
	            ...
	            did = new DocID(index,sDocId, databaseConfig.getDatabase(sDocId.substring(0, 3)));
	            docIds.add(did);
	            ...
	            totalDocCount = "1";
	            tQuery.setRecordCount(totalDocCount);
	            recnum = Integer.parseInt(currentRecord);
				totalcount = 1;

			/* 	UPTRefDocBuilder refBuilder = new UPTRefDocBuilder(new UPTRefDatabase());
				List prevNextDocids=refBuilder.getPrevNextDocIds("upt_146c1d4137723ec17f602b2061377553",3);
				if(prevNextDocids!=null && prevNextDocids.size()>0){
					prevDocId=(String)prevNextDocids.get(0);
					nextDocId=(String)prevNextDocids.get(1);
				}	 */
		...
		{
			absurl = getabsURL(tQuery,recnum,searchID,database,cid,sDocId,displayPagination);
		}
		else
		{
		...
		{
			deturl = getdetURL(tQuery,recnum,searchID,database,cid,sDocId,displayPagination);
		}
		else
		{
		...
		if(request.getAttribute("BOOKDET_URL") == null)
		{
			...
			if(request.getParameter("CID") != null && request.getParameter("CID").equals("pageDetailedFormat"))
			{
				bookdeturl = getbookdetURL(tQuery,recnum,searchID,database,did,pii,totalcount);
			}else{
		...
		if (request.getAttribute("RESULTS-COUNT") != null) {
			totalDocCount = (String) request.getAttribute("RESULTS-COUNT");
		}
...
//        FileWriter out1 = new FileWriter("C://baja/detail.xml");
       ...
       /*  if(prevDocId!= null){
        	out.write("<PREV-DOC-ID>"+prevDocId+"</PREV-DOC-ID>");
        }
    	if(nextDocId!= null){
        	out.write("<NEXT-DOC-ID>"+(nextDocId)+"</NEXT-DOC-ID>");
    	} */
    	out.write("<DISPLAY-PAGINATION>"+displayPagination+"</DISPLAY-PAGINATION>");
	...
	//entry.toXML(out1);
    ...
    } catch (EVBaseException e) {
        log4j.error("Unable to get abs/det page", e);
        ...
        client.setException("true");
        client.setRemoteControl();
        out.write((new SearchException(SystemErrorCodes.UNKNOWN_SEARCH_ERROR, e)).toXML());
        out.flush();
...
<%!
	...
	StringBuffer getabsURL(Query tQuery,int recnum,String searchID,String database,String cid,String sDocId,String displayPagination)
	{
		...
		absurlbuf.append("docid=").append(sDocId).append("&amp;");
		absurlbuf.append("database=").append(database).append("&amp;");
		absurlbuf.append("displayPagination=").append(displayPagination).append("&amp;");
	...
	StringBuffer getdetURL(Query tQuery,int recnum,String searchID,String database,String cid,String sDocId,String displayPagination)
	{
		...
		deturlbuf.append("docid=").append(sDocId).append("&amp;");
		deturlbuf.append("database=").append(database).append("&amp;");
		deturlbuf.append("displayPagination=").append(displayPagination).append("&amp;");
	...
	private StringBuffer getbookdetURL(Query tQuery,int recnum,String searchID,String database, DocID docID, String pii,int resultcount)
	{
		StringBuffer bookdeturlbuf = new StringBuffer();
		bookdeturlbuf.append("CID=bookSummary").append("&amp;");
		bookdeturlbuf.append("SEARCHID=").append(searchID).append("&amp;");
		...
		bookdeturlbuf.append("RESULTCOUNT=").append(Integer.toString(resultcount)).append("&amp;");
		bookdeturlbuf.append("docid=").append(docID.getDocID());
		return bookdeturlbuf;
	}

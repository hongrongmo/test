...
<%
	try {
        ...
        if (request.getParameter("pageSizeVal") != null) {
        ...
        if (request.getAttribute("SEARCH_RESULTS_URL") == null) {
            srurl = getsrURL(index, searchID, database, queryObject);
        } else {
        ...
        if (request.getAttribute("NEW_SEARCH_URL") == null) {
            nsurl = getnsURL(database, queryObject);
        } else {
        ...
        if (request.getAttribute("ABS_URL") == null) {
            absurl = getabsURL(queryObject, index, searchID, database);
        } else {
        ...
        if (request.getAttribute("DET_URL") == null) {
            deturl = getdetURL(queryObject, index, searchID, database);
        } else {
        ...
        RefPager pager = null;
        //System.out.println("DATABASE= "+(Integer.parseInt(database) & 2));
        //System.out.println("DATABASE1= "+database);
        if ((Integer.parseInt(database) & 2) == 2 && docid != null && docid.indexOf("inspec") > -1) {
            InspecDocBuilder docBuilder = new InspecDocBuilder(new InspecDatabase());
            List eiDocs = docBuilder.getRefPager(docid);
            curDoc = ((InspecWrapper) eiDocs.get(0)).eiDoc;
            eiDocs.remove(0);
            PatRefQuery refQuery = new PatRefQuery();
            pager = new RefPager(eiDocs, 25, refQuery, sessionId);
            refPage = pager.getPage(offset, eiDocs);
            ...
            pager = refBuilder.getRefPager(docid, pageSize, sessionId);

            if (request.getParameter("PAGE") != null) {
                offset = pageSize * (Integer.parseInt(request.getParameter("PAGE")) - 1);
            }
            refPage = pager.getPage(offset);
        ...
        //        databaseConfig.toXML(credentials, out1);
        out.write("<UPT-DOC>");
        //out1.write("<UPT-DOC>");
        curDoc.toXML(out);
        refPage.toXML(out);
        ...
        if (sRefQuery != null) {
            ...
            result = sc.openSearch(tQuery, sessionId, pageSize, false);
            ResultNavigator nav = sc.getNavigator();
            out.write(nav.toXML(tQuery.getResultsState()));
            //out1.write(nav.toXML(tQuery.getResultsState()));
        }

        out.write("<HIDE-NAV/>");
        if (queryObject != null) {
            out.write(queryObject.toXMLString());
            //out1.write(queryObject.toXMLString());
        } else {
...
<%!StringBuffer getsrURL(int recnum, String searchID, String database, Query tQuery) {
    ...
    private StringBuffer getabsURL(Query tQuery, int recnum, String searchID, String database) {
    ...
    private StringBuffer getnonpatrefURL(Query tQuery, int recnum, String searchID, String database, String docID) {
    ...
    private StringBuffer getdetURL(Query tQuery, int recnum, String searchID, String database) {
